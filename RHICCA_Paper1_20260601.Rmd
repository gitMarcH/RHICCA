...
datMiss %>%
  kable(caption="Numbers of non-missing data at the various visits for CRF, PWV, CIMT and ABPI.",col.names = c("Total","Baseline","6 month","12 months","18 months","24 months"), format = "html", escape = FALSE) %>%
...
kableRn<-function(.data,row_names=NULL, ...){
  if(!is.null(row_names)){
    ...
    res<-kable(datTmp, row.names=T, format = "html", escape = FALSE, ...)
  }else{
    res<-kable(.data, format = "html", escape = FALSE, ...)
  }
...
datTmp %>%
  ...
  kable(caption="Reasons for loss-to-follow-up at 24 months by participant type.",align = "l", format = "html", escape = FALSE) %>%
  kable_styling(full_width = F)
...
table(datTmp$crf.ptype,datTmp$ltfu_24months_ineligDetails) %>%
  t() %>%
  kable(caption="Reasons for retrospective ineligibility (participants with baseline CRF data only).", format = "html", escape = FALSE) %>%
  kable_styling(full_width = F)
...
cmvQuintiles<-quantile(probs=c(0,0.2,0.4,0.6,0.8,1),c(cmvDatWide$cmv.cmv_enrolment,cmvDatWide$cmv.cmv_6months),na.rm=TRUE)
vzvQuintiles<-quantile(probs=c(0,0.2,0.4,0.6,0.8,1),c(vzvDatWide$vzv.vzv_enrolment,vzvDatWide$vzv.vzv_6months),na.rm=TRUE)
...
cmvDatWide<-cmvDatWide %>%
  dplyr::mutate(
    ...
    cmvStatusQuint=factor(case_when(
      !is.na(cmv.cmv_enrolment) & !is.na(cmv.cmv_6months) & cmv.cmv_enrolment<cmvQuintiles["80%"] & cmv.cmv_6months<cmvQuintiles["80%"] ~ "persistently low titres",
      !is.na(cmv.cmv_enrolment) & !is.na(cmv.cmv_6months) & cmv.cmv_enrolment>=cmvQuintiles["80%"] & cmv.cmv_6months<cmvQuintiles["80%"] ~ "progression to low titres",
      !is.na(cmv.cmv_enrolment) & !is.na(cmv.cmv_6months) & cmv.cmv_enrolment<cmvQuintiles["80%"] & cmv.cmv_6months>=cmvQuintiles["80%"] ~ "progression to high titres",
      !is.na(cmv.cmv_enrolment) & !is.na(cmv.cmv_6months) & cmv.cmv_enrolment>=cmvQuintiles["80%"] & cmv.cmv_6months>=cmvQuintiles["80%"] ~ "persistently high titres",
      TRUE~NA_character_
    ),levels=c("persistently low titres","progression to low titres","progression to high titres","persistently high titres")),
  ...
  mutate(
        ...
        cmvStatusQuint_2levels=factor(case_when(
          !is.na(cmvStatusQuint) & cmvStatusQuint=="persistently high titres"~"persistently high titres",
          !is.na(cmvStatusQuint) & cmvStatusQuint!="persistently high titres"~"not persistently high titres"
        ),levels=c("not persistently high titres","persistently high titres"))
  )
...
vzvDatWide<-vzvDatWide %>%
  dplyr::mutate(
    ...
    vzvStatusQuint=factor(case_when(
      !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment<vzvQuintiles["80%"] & vzv.vzv_6months<vzvQuintiles["80%"] ~ "persistently low titres",
      !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment>=vzvQuintiles["80%"] & vzv.vzv_6months<vzvQuintiles["80%"] ~ "progression to low titres",
      !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment<vzvQuintiles["80%"] & vzv.vzv_6months>=vzvQuintiles["80%"] ~ "progression to high titres",
      !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment>=vzvQuintiles["80%"] & vzv.vzv_6months>=vzvQuintiles["80%"] ~ "persistently high titres",
      TRUE~NA_character_
    ),levels=c("persistently low titres","progression to low titres","progression to high titres","persistently high titres")),
  ...
  mutate(
        ...
        vzvStatusQuint_2levels=factor(case_when(
          !is.na(vzvStatusQuint) & vzvStatusQuint=="persistently high titres"~"persistently high titres",
          !is.na(vzvStatusQuint) & vzvStatusQuint!="persistently high titres"~"not persistently high titres"
        ),levels=c("not persistently high titres","persistently high titres"))
  )
...
# renaming some variables for the complete case analyses XXYY
rhiccaDatWide<-rhiccaDatWide %>%
  dplyr::mutate(
    numClinEventsPerParticipantCIMTonly=rhiccaDatWide$numClinEventsCimt,
    numClinEventsPerParticipantCVDwithCIMT=rhiccaDatWide$numClinEventsPerParticipantCVD+rhiccaDatWide$numClinEventsCimt,
    lims.CD4_Absolute_changeFromBaselineAt24Months=lims.CD4_Absolute_24months-lims.CD4_Absolute_enrolment
  )
...
write.csv(rhiccaDatLong,file=paste(sep="","../output/",gsub(pattern="-",replacement="",Sys.Date()),"/rhiccaDat_allMerged_longFormat",gsub(pattern="-",replacement="",Sys.Date()),".csv"))
...
demoDatKable %>%
  kable(col.names=rep("",ncol(demoDatKable)),caption="Participant characteristics (at baseline).", format = "html", escape = FALSE) %>%
  kable_styling(full_width=F) %>%
...
summary(pool(obj1GlmCimt.mid)) %>%
  ...
  kableRn(caption="Model CIMT-1: Results for the linear regression model for change from baseline CIMT with participant type and baseline CIMT as predictors. Model fitted to multiply imputed data.",digits=4,row_names=c("Intercept","Participant type: PLWH","Baseline CIMT")) %>%
  kable_styling(full_width=F)
```
...
tabAll %>%
  dplyr::select(!term) %>%
  kableRn(digits=4,row_names=rn,col.names=rep(c("Coefficient","SE","p"),3),caption="Model summaries for all models fitted to CIMT as outcome (unadjusted, single variable models for all participants, multiple regression model for all participants and multiple regression model for PLWH only.)") %>%
...
sumFitandSum_cimt.hiv("-cimt.cimt_enrolment") %>%
  ...
  kableRn(digits=3,row_names=c("(Intercept)"),col.names=c("Coefficient","SE","p"),caption="Model summary for the basic unadjusted model fitted to CIMT as outcome. Model fitted to PLWH only.") %>%
  #pack_rows(index = c("Unadjusted models, all data" = 3, "Multiple regression model, all data" = 16, "Multiple regression model, PLWH only" = 18)) %>%
  kable_styling(full_width=F)
```
...
tab3 %>%
  ...
  dplyr::select(!c(term,rownum,statistic,df)) %>%
  kableRn(digits=3,row_names=c("(Intercept)"),col.names=c("Coefficient","SE","p"),caption="Model summary (showing intercept only!) for the fully adjusted model fitted to CIMT as outcome. Model fitted to PLWH only.") %>%
  #pack_rows(index = c("Unadjusted models, all data" = 3, "Multiple regression model, all data" = 16, "Multiple regression model, PLWH only" = 18)) %>%
  kable_styling(full_width=F)
```
...
sumFitandSum_cimt("-cimt.cimt_enrolment") %>%
  dplyr::select(!c(term,rownum,statistic,df)) %>%
  kableRn(digits=3,row_names=c("(Intercept)"),col.names=c("Coefficient","SE","p"),caption="Model summary for the basic unadjusted model fitted to CIMT as outcome. Model fitted to all participants.") %>%
  #pack_rows(index = c("Unadjusted models, all data" = 3, "Multiple regression model, all data" = 16, "Multiple regression model, PLWH only" = 18)) %>%
  kable_styling(full_width=F)
```
...
tab2 %>%
  dplyr::filter(term=="(Intercept)") %>%
  dplyr::select(!c(term,rownum,statistic,df)) %>%
  kableRn(digits=3,row_names=c("(Intercept)"),col.names=c("Coefficient","SE","p"),caption="Model summary (showing intercept only!) for the fully adjusted model fitted to CIMT as outcome. Model fitted to all participants.") %>%
  #pack_rows(index = c("Unadjusted models, all data" = 3, "Multiple regression model, all data" = 16, "Multiple regression model, PLWH only" = 18)) %>%
  kable_styling(full_width=F)
```
...
tabAll %>%
  dplyr::select(!term) %>%
  kableRn(digits=3,row_names=rn,col.names=rep(c("Coefficient","SE","p"),2),caption="Model summaries for all models fitted to CIMT as outcome (unadjusted, single variable models and multiple regression model.). Models fitted to PLWH only.") %>%
  #pack_rows(index = c("Unadjusted models, all data" = 3, "Multiple regression model, all data" = 16, "Multiple regression model, PLWH only" = 18)) %>%
...
```{r}
tabAlt %>%
  kableRn(digits=4,row_names=rnAlt,caption="Model summaries for 2 additional models fitted to CIMT as outcome (adjusted model for all participants and PLWH only where adjustment is only for CMV and VZV status.)") %>%
...
```{r}
tabAlt2 %>%
  kableRn(digits=4,row_names=rnAlt2,caption="Model summaries for another 2 additional models fitted to CIMT as outcome (adjusted model for all participants and PLWH only where adjustment is for the continuous CMV and VZV measurements.)") %>%
...
tabAll %>%
  dplyr::select(!term) %>%
  kableRn(digits=4,row_names=rn,col.names=rep(c("Coefficient","SE","p"),3),caption="Model summaries for all mixed effects models fitted to PWV as outcome (unadjusted, single variable models for all participants, multiple regression model for all participants and multiple regression model for PLWH only.)") %>%
  #pack_rows(index = c("Unadjusted models, all data" = 3, "Multiple regression model, all data" = 16, "Multiple regression model, PLWH only" = 18)) %>%
  add_header_above(header=c(" "=1,"Unadjusted models, all data" = 3, "Multiple regression model, all data" = 3, "Multiple regression model, PLWH" = 3)) %>%
  kable_styling(full_width=F)
...
tabAll %>%
  dplyr::select(!term) %>%
  kableRn(digits=4,row_names=rn,col.names=rep(c("Coefficient","SE","p"),2),caption="Model summaries for all models fitted to PWV as outcome (unadjusted, single variable models and multiple regression model.). Models fitted to PLWH only.") %>%
  add_header_above(header=c(" "=1,"Unadjusted models" = 3, "Multiple regression model" = 3)) %>%
  kable_styling(full_width=F)
...
tab3 %>%
  dplyr::filter(term=="(Intercept)") %>%
  dplyr::select(!c(term,rownum,statistic,df)) %>%
  kableRn(digits=3,row_names=c("(Intercept)"),col.names=c("Coefficient","SE","p"),caption="Model summary (showing intercept only!) for the fully adjusted model fitted to CIMT as outcome. Model fitted to PLWH only but including ART treatment.") %>%
  #pack_rows(index = c("Unadjusted models, all data" = 3, "Multiple regression model, all data" = 16, "Multiple regression model, PLWH only" = 18)) %>%
  kable_styling(full_width=F)
```
...
kable(tt,caption=" Clinical and sub-clinical events summarised by different participant characteristics. The numbers in brackets are the event rates per 1,000 person-years.", format = "html", escape = FALSE) %>%
...
```{r}
pafBootStrapCI<-function(clinEvents,dat,B=1e3,seed=123){
  clinEvents<-clinEvents %>%
    dplyr::filter(category %in% c("CVD","CIMT","PWV"))
  
  clinEvents$category<-factor(clinEvents$category,levels=c("CIMT","PWV","CVD"))
  
  set.seed(seed)
  
  pafMat<-data.frame(
    pafClinical=rep(NA,B),
    pafCimt=rep(NA,B),
    pafPwv=rep(NA,B),
    pafClinCimt=rep(NA,B)
  )
  
  for(b in 1:B){
    rhiccaDatWideBS<-dat[sample(x=1:nrow(dat),size=nrow(dat),replace=TRUE),]
    clinEventsTmpBS<-clinEvents[match(rhiccaDatWideBS$pid,clinEvents$pid),] %>% dplyr::filter(!is.na(category))
    
    tt<-table(clinEventsTmpBS$category)
    ttt<-data.frame(
      CVD.clinical=tt["CVD"],
      CVD.CIMT=tt["CIMT"],
      CVD.PWV=tt["PWV"]
    ...
    ttCount<-tt
    tt<-tt/sum(as.numeric(rhiccaDatWideBS$obsTime)/(1000*365.25))
    ttLow=tt-qnorm(0.975)*sqrt(tt/sum(as.numeric(rhiccaDatWideBS$obsTime)/(1000*365.25)))
    ttUpp=tt+qnorm(0.975)*sqrt(tt/sum(as.numeric(rhiccaDatWideBS$obsTime)/(1000*365.25)))
    ...
    tt<-table(clinEventsTmpBS$ptype,clinEventsTmpBS$category)
    ttt<-data.frame(
      CVD.clinical=tt[,"CVD"],
      CVD.CIMT=tt[,"CIMT"],
      CVD.PWV=tt[,"PWV"]
    ...
    ttUpp<-tt
    for(i in 1:nrow(ttCount)){
      tt[i,]<-tt[i,]/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH==rownames(tt)[i]])/(1000*365.25))
      ttLow[i,]=tt[i,]-qnorm(0.975)*sqrt(tt[i,]/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH==rownames(tt)[i]])/(1000*365.25)))
      ttUpp[i,]=tt[i,]+qnorm(0.975)*sqrt(tt[i,]/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH==rownames(tt)[i]])/(1000*365.25)))
    }
    ...
    cvdClinicalPoisP<-poisson.test(x=ttCountCount[,"CVD.clinical"],T=c(sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH==rownames(tt)[1]])/(1000*365.25)),sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH==rownames(tt)[2]])/(1000*365.25))))$p.value
    
    totCases<-sum(ttCountCount[,"CVD.clinical"])
    incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="PLWH","CVD.clinical"])/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH!="PLWH"])/(1000*365.25))
    hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="PLWH","CVD.clinical"])+incNonExp*sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH=="PLWH"])/(1000*365.25))
    pafMat$pafClinical[b]<-(totCases-hypoCases)/totCases
    
    cvdCimtPoisP<-poisson.test(x=ttCountCount[,"CVD.CIMT"],T=c(sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH==rownames(tt)[1]])/(1000*365.25)),sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH==rownames(tt)[2]])/(1000*365.25))))$p.value
    
    totCases<-sum(ttCountCount[,"CVD.CIMT"])
    incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="PLWH","CVD.CIMT"])/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH!="PLWH"])/(1000*365.25))
    hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="PLWH","CVD.CIMT"])+incNonExp*sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH=="PLWH"])/(1000*365.25))
    pafMat$pafCimt[b]<-(totCases-hypoCases)/totCases
    
    cvdPwvPoisP<-poisson.test(x=ttCountCount[,"CVD.PWV"],T=c(sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH==rownames(tt)[1]])/(1000*365.25)),sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH==rownames(tt)[2]])/(1000*365.25))))$p.value
    
    totCases<-sum(ttCountCount[,"CVD.PWV"])
    incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="PLWH","CVD.PWV"])/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH!="PLWH"])/(1000*365.25))
    hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="PLWH","CVD.PWV"])+incNonExp*sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH=="PLWH"])/(1000*365.25))
    pafMat$pafPwv[b]<-(totCases-hypoCases)/totCases
    
    cvdClinicalCimtPoisP<-poisson.test(x=ttCountCount[,"CVD.clinicalAndCIMT"],T=c(sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH==rownames(tt)[1]])/(1000*365.25)),sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH==rownames(tt)[2]])/(1000*365.25))))$p.value
    
    totCases<-sum(ttCountCount[,"CVD.clinicalAndCIMT"])
    incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="PLWH","CVD.clinicalAndCIMT"])/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH!="PLWH"])/(1000*365.25))
    hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="PLWH","CVD.clinicalAndCIMT"])+incNonExp*sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.PLWH=="PLWH"])/(1000*365.25))
    pafMat$pafClinCimt[b]<-(totCases-hypoCases)/totCases
  }
  
  res<-list(
    pafClinicalCI=HDInterval::hdi(object=pafMat$pafClinical,credMass=0.95),
    pafCimtCI=HDInterval::hdi(object=pafMat$pafCimt,credMass=0.95),
    pafPwvCI=HDInterval::hdi(object=pafMat$pafPwv,credMass=0.95),
    pafClinCimtCI=HDInterval::hdi(object=pafMat$pafClinCimt,credMass=0.95),
    bsMat=pafMat
  )
...
if(!skipPafBs){
  pafCIs<-pafBootStrapCI(clinEvents=clinEventsTmp,dat=rhiccaDatWide,B=NpafBS)
  save(pafCIs,file=paste(sep="","../output/",gsub(pattern="-",replacement="",Sys.Date()),"/rhiccaDat_pafCIs_",gsub(pattern="-",replacement="",Sys.Date()),".RData"))
}else{
  load(paste(sep="","../output/",pafBsDate2use,"/rhiccaDat_pafCIs_",pafBsDate2use,".RData"))
}
```
...
pafClinical<-paste(sep="",format(nsmall=3,round(digits=3,pafClinical))," (",format(nsmall=3,round(digits=3,pafCIs$pafClinicalCI[1])),",",format(nsmall=3,round(digits=3,pafCIs$pafClinicalCI[2])),")")
pafCimt<-paste(sep="",format(nsmall=3,round(digits=3,pafCimt))," (",format(nsmall=3,round(digits=3,pafCIs$pafCimtCI[1])),",",format(nsmall=3,round(digits=3,pafCIs$pafCimtCI[2])),")")
pafPwv<-paste(sep="",format(nsmall=3,round(digits=3,pafPwv))," (",format(nsmall=3,round(digits=3,pafCIs$pafPwvCI[1])),",",format(nsmall=3,round(digits=3,pafCIs$pafPwvCI[2])),")")
pafClinCimt<-paste(sep="",format(nsmall=3,round(digits=3,pafClinCimt))," (",format(nsmall=3,round(digits=3,pafCIs$pafClinCimt[1])),",",format(nsmall=3,round(digits=3,pafCIs$pafClinCimtCI[2])),")")

ttCountAll<-rbind(ttCountAll,paste(sep="","Poisson test p = ",format(nsmall=3,round(digits=3,c(cvdClinicalPoisP,cvdCimtPoisP,cvdPwvPoisP,cvdClinicalCimtPoisP))),", PAF = ",c(pafClinical,pafCimt,pafPwv,pafClinCimt)),ttCount)
```

```{r}
pafAgeBootStrapCI<-function(clinEvents,dat,B=1e3,seed=123){
  clinEvents<-clinEvents %>%
    dplyr::filter(category %in% c("CVD","CIMT","PWV")) %>%
    dplyr::mutate(
  ...
  for(b in 1:B){
    ...
    while(length(unique(clinEventsTmpBS$age_group_2lvls))<=1){
      rhiccaDatWideBS<-dat[sample(x=1:nrow(dat),size=nrow(dat),replace=TRUE),]
      clinEventsTmpBS<-clinEvents[match(rhiccaDatWideBS$pid,clinEvents$pid),] %>% dplyr::filter(!is.na(category))
    }
    
    tt<-table(clinEventsTmpBS$age_group_2lvls,clinEventsTmpBS$category)
    ttt<-data.frame(
      CVD.clinical=tt[,"CVD"],
      CVD.CIMT=tt[,"CIMT"],
      CVD.PWV=tt[,"PWV"]
    ...
    for(i in 1:nrow(ttCount)){
      if(i==1){
        tt[i,]<-tt[i,]/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age<55])/(1000*365.25))
        ttLow[i,]=tt[i,]-qnorm(0.975)*sqrt(tt[i,]/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age<55])/(1000*365.25)))
        ttUpp[i,]=tt[i,]+qnorm(0.975)*sqrt(tt[i,]/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age<55])/(1000*365.25)))
      }else if(i==2){
        tt[i,]<-tt[i,]/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age>=55])/(1000*365.25))
        ttLow[i,]=tt[i,]-qnorm(0.975)*sqrt(tt[i,]/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age>=55])/(1000*365.25)))
        ttUpp[i,]=tt[i,]+qnorm(0.975)*sqrt(tt[i,]/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age>=55])/(1000*365.25)))
    ...
    if(length(ttCountCount[,"CVD.clinical"])==1){
      print(ttCountCount)
      print(unique(clinEventsTmpBS$age_group_2lvls))
    }
    
    cvdClinicalPoisP<-poisson.test(x=ttCountCount[,"CVD.clinical"],T=c(sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age<55])/(1000*365.25)),sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age>=55])/(1000*365.25))))$p.value
    
    totCases<-sum(ttCountCount[,"CVD.clinical"])
    incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="55+","CVD.clinical"])/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age<55])/(1000*365.25))
    hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="55+","CVD.clinical"])+incNonExp*sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age>=55])/(1000*365.25))
    pafMat$pafClinical[b]<-(totCases-hypoCases)/totCases
    
    cvdCimtPoisP<-poisson.test(x=ttCountCount[,"CVD.CIMT"],T=c(sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age<55])/(1000*365.25)),sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age>=55])/(1000*365.25))))$p.value
    
    totCases<-sum(ttCountCount[,"CVD.CIMT"])
    incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="55+","CVD.CIMT"])/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age<55])/(1000*365.25))
    hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="55+","CVD.CIMT"])+incNonExp*sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age>=55])/(1000*365.25))
    pafMat$pafCimt[b]<-(totCases-hypoCases)/totCases
    
    cvdPwvPoisP<-poisson.test(x=ttCountCount[,"CVD.PWV"],T=c(sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age<55])/(1000*365.25)),sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age>=55])/(1000*365.25))))$p.value
    
    totCases<-sum(ttCountCount[,"CVD.PWV"])
    incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="55+","CVD.PWV"])/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age<55])/(1000*365.25))
    hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="55+","CVD.PWV"])+incNonExp*sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age>=55])/(1000*365.25))
    pafMat$pafPwv[b]<-(totCases-hypoCases)/totCases
    
    cvdClinicalCimtPoisP<-poisson.test(x=ttCountCount[,"CVD.clinicalAndCIMT"],T=c(sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age<55])/(1000*365.25)),sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age>=55])/(1000*365.25))))$p.value
    
    totCases<-sum(ttCountCount[,"CVD.clinicalAndCIMT"])
    incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="55+","CVD.clinicalAndCIMT"])/sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age<55])/(1000*365.25))
    hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="55+","CVD.clinicalAndCIMT"])+incNonExp*sum(as.numeric(rhiccaDatWideBS$obsTime[rhiccaDatWideBS$crf.age>=55])/(1000*365.25))
    pafMat$pafClinCimt[b]<-(totCases-hypoCases)/totCases
  }
...
if(!skipPafBs){
  pafAgeCIs<-pafAgeBootStrapCI(clinEvents=clinEventsTmp,dat=rhiccaDatWide,B=NpafBS)
  save(pafAgeCIs,file=paste(sep="","../output/",gsub(pattern="-",replacement="",Sys.Date()),"/rhiccaDat_pafAgeCIs_",gsub(pattern="-",replacement="",Sys.Date()),".RData"))
}else{
  load(paste(sep="","../output/",pafBsDate2use,"/rhiccaDat_pafAgeCIs_",pafBsDate2use,".RData"))
}
```
...
pafClinical<-paste(sep="",format(nsmall=3,round(digits=3,pafClinical))," (",format(nsmall=3,round(digits=3,pafAgeCIs$pafClinicalCI[1])),",",format(nsmall=3,round(digits=3,pafAgeCIs$pafClinicalCI[2])),")")
pafCimt<-paste(sep="",format(nsmall=3,round(digits=3,pafCimt))," (",format(nsmall=3,round(digits=3,pafAgeCIs$pafCimtCI[1])),",",format(nsmall=3,round(digits=3,pafAgeCIs$pafCimtCI[2])),")")
pafPwv<-paste(sep="",format(nsmall=3,round(digits=3,pafPwv))," (",format(nsmall=3,round(digits=3,pafAgeCIs$pafPwvCI[1])),",",format(nsmall=3,round(digits=3,pafAgeCIs$pafPwvCI[2])),")")
pafClinCimt<-paste(sep="",format(nsmall=3,round(digits=3,pafClinCimt))," (",format(nsmall=3,round(digits=3,pafAgeCIs$pafClinCimt[1])),",",format(nsmall=3,round(digits=3,pafAgeCIs$pafClinCimtCI[2])),")")

ttCountAll<-rbind(ttCountAll,paste(sep="","Poisson test p = ",format(nsmall=3,round(digits=3,c(cvdClinicalPoisP,cvdCimtPoisP,cvdPwvPoisP,cvdClinicalCimtPoisP))),", PAF = ",c(pafClinical,pafCimt,pafPwv,pafClinCimt)),ttCount)
```

```{r}
pafVzvBootStrapCI<-function(clinEvents,dat,dat_impMice.clinEventsConfirmed,B=1e3,seed=123){
  pidsWithVzv<-dat$pid[!is.na(dat$vzvStatus)]
  ...
  for(b in 1:B){
    rhiccaDatWideBS<-dat[sample(x=1:nrow(dat),size=nrow(dat),replace=TRUE),]
    capture.output(
      rhiccaDatWide_impMice.clinEventsConfirmedBS<-subset_datlist(dat_impMice.clinEventsConfirmed, subset = match(rhiccaDatWideBS$pid,dat$pid), toclass = "mids")
    )
    clinEventsTmpBS<-clinEvents[match(rhiccaDatWideBS$pid,clinEvents$pid),] %>% dplyr::filter(!is.na(category))
    
    tt<-table(clinEventsTmpBS$vzvStatus,clinEventsTmpBS$category)
    ttt<-data.frame(
      CVD.clinical=tt[,"CVD"],
      CVD.CIMT=tt[,"CIMT"],
      CVD.PWV=tt[,"PWV"]
    ...
    ttUpp<-tt
    for(i in 1:nrow(ttCount)){
      tt[i,]<-tt[i,]/sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus) & rhiccaDatWideBS$vzvStatus==rownames(tt)[i]])/(1000*365.25))
      ttLow[i,]=tt[i,]-qnorm(0.975)*sqrt(tt[i,]/sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus) & rhiccaDatWideBS$vzvStatus==rownames(tt)[i]])/(1000*365.25)))
      ttUpp[i,]=tt[i,]+qnorm(0.975)*sqrt(tt[i,]/sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus) & rhiccaDatWideBS$vzvStatus==rownames(tt)[i]])/(1000*365.25)))
    }
    ...
    modClinEventsConfirmedCVD.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmedBS,glm(numClinEventsPerParticipantCVD~vzvStatus,family="quasipoisson",offset=log(obsTime/(1000*365.25)+1)))
    modClinEventsCIMTonly.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmedBS,glm(numClinEventsPerParticipantCIMTonly~vzvStatus,family="quasipoisson",offset=log(obsTime/(1000*365.25)+1)))
    modClinEventsPWVonly.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmedBS,glm(numClinEventsPerParticipantPWVonly~vzvStatus,family="quasipoisson",offset=log(obsTime/(1000*365.25)+1)))
    modClinEventsConfirmedCVDwithCIMT.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmedBS,glm(numClinEventsPerParticipantCVDwithCIMT~vzvStatus,family="quasipoisson",offset=log(obsTime/(1000*365.25)+1)))
    modClinEventsConfirmedCVDwithCIMT.univariable.2levels<-with(rhiccaDatWide_impMice.clinEventsConfirmedBS,glm(numClinEventsPerParticipantCVDwithCIMT~vzvStatus_2levels,family="quasipoisson",offset=log(obsTime/(1000*365.25)+1)))
    ...
    totCases<-sum(ttCountCount[,"CVD.clinical"])
    incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="persistently high titres","CVD.clinical"])/sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus) & rhiccaDatWideBS$vzvStatus!="persistently high titres"])/(1000*365.25))
    hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="persistently high titres","CVD.clinical"])+incNonExp*sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus) & rhiccaDatWideBS$vzvStatus=="persistently high titres"])/(1000*365.25))
    pafMat$pafClinical[b]<-(totCases-hypoCases)/totCases
    ...
    totCases<-sum(ttCountCount[,"CVD.CIMT"])
    incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="persistently high titres","CVD.CIMT"])/sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus) & rhiccaDatWideBS$vzvStatus!="persistently high titres"])/(1000*365.25))
    hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="persistently high titres","CVD.CIMT"])+incNonExp*sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus) & rhiccaDatWideBS$vzvStatus=="persistently high titres"])/(1000*365.25))
    pafMat$pafCimt[b]<-(totCases-hypoCases)/totCases
    ...
    totCases<-sum(ttCountCount[,"CVD.PWV"])
    incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="persistently high titres","CVD.PWV"])/sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus) & rhiccaDatWideBS$vzvStatus!="persistently high titres"])/(1000*365.25))
    hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="persistently high titres","CVD.PWV"])+incNonExp*sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus) & rhiccaDatWideBS$vzvStatus=="persistently high titres"])/(1000*365.25))
    pafMat$pafPwv[b]<-(totCases-hypoCases)/totCases
    ...
    totCases<-sum(ttCountCount[,"CVD.clinicalAndCIMT"])
    incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="persistently high titres","CVD.clinicalAndCIMT"])/sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus) & rhiccaDatWideBS$vzvStatus!="persistently high titres"])/(1000*365.25))
    incExp<-sum(ttCountCount[rownames(ttCountCount)=="persistently high titres","CVD.clinicalAndCIMT"])/sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus) & rhiccaDatWideBS$vzvStatus=="persistently high titres"])/(1000*365.25))
    incOverall<-sum(ttCountCount[,"CVD.clinicalAndCIMT"])/sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus)])/(1000*365.25))
    hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="persistently high titres","CVD.clinicalAndCIMT"])+incNonExp*sum(as.numeric(rhiccaDatWideBS$obsTime[!is.na(rhiccaDatWideBS$vzvStatus) & rhiccaDatWideBS$vzvStatus=="persistently high titres"])/(1000*365.25))
    pafMat$pafClinCimt[b]<-(totCases-hypoCases)/totCases
  }
...
if(!skipPafBs){
  pafVzvCIs<-pafVzvBootStrapCI(clinEvents=clinEventsTmp,dat=rhiccaDatWide,dat_impMice.clinEventsConfirmed=rhiccaDatWide_impMice.clinEventsConfirmed,B=NpafBS)
  save(pafVzvCIs,file=paste(sep="","../output/",gsub(pattern="-",replacement="",Sys.Date()),"/rhiccaDat_pafVzvCIs_",gsub(pattern="-",replacement="",Sys.Date()),".RData"))
}else{
  load(paste(sep="","../output/",pafBsDate2use,"/rhiccaDat_pafVzvCIs_",pafBsDate2use,".RData"))
}
```
...
pafClinical<-paste(sep="",format(nsmall=3,round(digits=3,pafClinical))," (",format(nsmall=3,round(digits=3,pafVzvCIs$pafClinicalCI[1])),",",format(nsmall=3,round(digits=3,pafVzvCIs$pafClinicalCI[2])),")")
pafCimt<-paste(sep="",format(nsmall=3,round(digits=3,pafCimt))," (",format(nsmall=3,round(digits=3,pafVzvCIs$pafCimtCI[1])),",",format(nsmall=3,round(digits=3,pafVzvCIs$pafCimtCI[2])),")")
pafPwv<-paste(sep="",format(nsmall=3,round(digits=3,pafPwv))," (",format(nsmall=3,round(digits=3,pafVzvCIs$pafPwvCI[1])),",",format(nsmall=3,round(digits=3,pafVzvCIs$pafPwvCI[2])),")")
pafClinCimt<-paste(sep="",format(nsmall=3,round(digits=3,pafClinCimt))," (",format(nsmall=3,round(digits=3,pafVzvCIs$pafClinCimt[1])),",",format(nsmall=3,round(digits=3,pafVzvCIs$pafClinCimtCI[2])),")")

ttCountAll<-rbind(ttCountAll,paste(sep="","Poisson regression p-value for persistently high VZV = ",format(nsmall=3,round(digits=3,c(cvdClinicalPoisP,cvdCimtPoisP,cvdPwvPoisP,cvdClinicalCimtPoisP))),", PAF = ",c(pafClinical,pafCimt,pafPwv,pafClinCimt)),ttCount)
```
...
kable(ttCountAll,caption="CVD clinical and sub-clinical events in participants. The number in brackets is the rate of events per 1,000 person-years.",col.names=c("Clinical events","CIMT sub-clinical events","PWV sub-clinical events","Clinical & CIMT sub-clinical events"), format = "html", escape = FALSE) %>%
  kable_styling(full_width = FALSE) %>%
...
```{r}
if(!skipPafBs){
  pafAgeCIs<-pafAgeBootStrapCI(clinEvents=clinEventsTmpPLWH,dat=rhiccaDatWide %>% dplyr::filter(crf.PLWH=="PLWH"),B=NpafBS)
  save(pafAgeCIs,file=paste(sep="","../output/",gsub(pattern="-",replacement="",Sys.Date()),"/rhiccaDat_pafAgeCIs_PLWH_",gsub(pattern="-",replacement="",Sys.Date()),".RData"))
}else{
  load(paste(sep="","../output/",pafBsDate2use,"/rhiccaDat_pafAgeCIs_PLWH_",pafBsDate2use,".RData"))
}
...
ttCountAll<-rbind(ttCountAll,paste(sep="","Poisson test p = ",format(nsmall=3,round(digits=3,c(cvdClinicalPoisP,cvdCimtPoisP,cvdClinicalCimtPoisP))),", PAF = ",c(pafClinical,pafCimt,pafClinCimt)),ttCount)
```
...
```{r}
if(!skipPafBs){
  pafVzvCIs<-pafVzvBootStrapCI(clinEvents=clinEventsTmpPLWH,dat=rhiccaDatWide %>% dplyr::filter(crf.PLWH=="PLWH"),dat_impMice.clinEventsConfirmed=rhiccaDatWide_impMice.clinEventsConfirmed %>% dplyr::filter(crf.ptype=="Patient"),B=NpafBS)
  save(pafVzvCIs,file=paste(sep="","../output/",gsub(pattern="-",replacement="",Sys.Date()),"/rhiccaDat_pafVzvCIs_PLWH_",gsub(pattern="-",replacement="",Sys.Date()),".RData"))
}else{
  load(paste(sep="","../output/",pafBsDate2use,"/rhiccaDat_pafVzvCIs_PLWH_",pafBsDate2use,".RData"))
}
...
ttCountAll<-rbind(ttCountAll,paste(sep="","Poisson regression p-value for persistently high VZV = ",format(nsmall=3,round(digits=3,c(cvdClinicalPoisP,cvdCimtPoisP,cvdClinicalCimtPoisP))),", PAF = ",c(pafClinical,pafCimt,pafClinCimt)),ttCount)
```

Note: alternative way of computing a CI for PAF ClinCimt: `r paste(sep=" ",pafClinCimtL,pafClinCimtU)` - compare this with `r pafClinCimt`.
...
kable(ttCountAll,caption="CVD clinical and sub-clinical events in PLWH. The number in brackets is the rate of events per 1,000 person-years.",col.names=c("Clinical events","CIMT sub-clinical events","Clinical & CIMT sub-clinical events"), format = "html", escape = FALSE) %>%
  kable_styling(full_width = FALSE) %>%
  pack_rows(start_row=1,end_row=1,group_label="All participants.") %>%
...
funFitAndSum_modClinEventsConfirmedCVDwithCIMT.cc<-function(varString,filterPLWH=TRUE,nDigits=2){
  if(filterPLWH){
    datTmp<-rhiccaDatWide %>% dplyr::filter(crf.ptype=="Patient")
  }else{
    datTmp<-rhiccaDatWide
  }
  mod<-glm(as.formula(paste(sep="","numClinEventsPerParticipantCVDwithCIMT~",varString)),data=datTmp,offset=as.numeric(datTmp$obsTime/(1000*365.25)),family="quasipoisson")
  
  sumObj<-summary(mod)$coefficients %>%
    as.data.frame() %>%
    mutate(rownum=1:nrow(summary(mod)$coefficients)) %>%
    mutate(
      estimateLink=Estimate,
      estimate=format(nsmall=nDigits,round(digits=nDigits,exp(Estimate))),
      rrCI=NA
  ...
  colnames(sumObj)[colnames(sumObj)=="Pr(>|t|)"]<-"p.value"
  colnames(sumObj)[colnames(sumObj)=="Std. Error"]<-"std.error"
...
funFitAndSum_modClinEventsCIMTonly.cc<-function(varString,filterPLWH=TRUE,nDigits=2){
  if(filterPLWH){
    datTmp<-rhiccaDatWide %>% dplyr::filter(crf.ptype=="Patient")
  }else{
    datTmp<-rhiccaDatWide
  }
  mod<-glm(as.formula(paste(sep="","numClinEventsPerParticipantCIMTonly~",varString)),data=datTmp,offset=as.numeric(datTmp$obsTime/(1000*365.25)),family="quasipoisson")
...
tab2.quint<-funFitAndSum_modClinEventsCIMTonly("crf.gender+crf.age+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatusQuint_2levels+vzvStatusQuint_2levels+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months+log10(lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment)")
tab2.quint$term<-as.character(tab2.quint$term)

tab2.cimtonly.cc<-funFitAndSum_modClinEventsCIMTonly.cc("crf.gender+crf.age+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months+log10(lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment)")
tab2.cimtonly.cc$term<-rownames(tab2.cimtonly.cc)

tab2.cimtonly.quint.cc<-funFitAndSum_modClinEventsCIMTonly.cc("crf.gender+crf.age+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatusQuint_2levels+vzvStatusQuint_2levels+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months+log10(lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment)")
tab2.cimtonly.quint.cc$term<-rownames(tab2.cimtonly.quint.cc)

tab2.cimtonly.all.cc<-funFitAndSum_modClinEventsCIMTonly.cc(filterPLWH=FALSE,"crf.ptype+crf.gender+crf.age+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels")
tab2.cimtonly.all.cc$term<-rownames(tab2.cimtonly.all.cc)
...
tab2.cvdwithcimt.cc<-funFitAndSum_modClinEventsConfirmedCVDwithCIMT.cc("crf.gender+crf.age+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months+log10(lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment)")
tab2.cvdwithcimt.cc$term<-rownames(tab2.cvdwithcimt.cc)

tab2.cvdwithcimt.all.cc<-funFitAndSum_modClinEventsConfirmedCVDwithCIMT.cc(filterPLWH=FALSE,"crf.ptype+crf.gender+crf.age+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels")
tab2.cvdwithcimt.all.cc$term<-rownames(tab2.cvdwithcimt.all.cc)
...
#### Sensitivity analysis - all participants complete case analysis

**CIMT only**

```{r}
tab2.cimtonly.all.cc %>%
  kable(format="html", escape = FALSE) %>%
  kable_styling()
```
...
```{r}
tab2.cvdwithcimt.all.cc %>%
  kable(format="html", escape = FALSE) %>%
  kable_styling()
```

#### Sensitivity analysis - PLWH complete case analysis

**CIMT only**

```{r}
print(tab2.cimtonly.cc) %>%
  kable(format="html", escape = FALSE) %>%
  kable_styling()
```
...
```{r}
print(tab2.cvdwithcimt.cc) %>%
  kable(format="html", escape = FALSE) %>%
  kable_styling()
```

#### Sensitivity analysis - PLWH, multiply imputes but using quintiles for CMV / VZV status definitions

**Multiply-imputed + CMV/VZV quintiles**
...
**Complete-case + CMV/VZV quintiles**

```{r}
print(tab2.cimtonly.quint.cc) %>%
  kable(format="html", escape = FALSE) %>%
  kable_styling()
```
...
tabAll.nocd4 %>%
  ...
  kableRn(digits=3,row_names=rn.nocd4,col.names=c(rep(c("RR","95% CI","p"),2),rep(c("RR","95% CI","p"),4)),caption="Model summaries for models for PLWH only 1) fitted to clinical CVD events only, 2) fitted to sub-clinical CIMT events only and 3) fitted to clinical CVD events and sub-clinical CIMT events (both unadjusted, single variable models and multiple regression models). The event count models are Poisson models and hence report rate ratios. Models fitted to multiply-imputed data from PLWH only.") %>%
  #pack_rows(index = c("Unadjusted models, all data" = 3, "Multiple regression model, all data" = 16, "Multiple regression model, PLWH only" = 18)) %>%
  add_header_above(header=c(" "=1,"Unadjusted models" = 3, "Multiple regression model" = 3,"Unadjusted models" = 3, "Multiple regression model" = 3,"Unadjusted models" = 3, "Multiple regression model" = 3)) %>%
  add_header_above(header=c(" "=1,"Clinical CVD events" = 6, "Sub-clinical CIMT events" = 6,"Clinical CVD and sub-clinical CIMT events" = 6)) %>%
  kable_styling(full_width=F)
...
funFitAndSum_modClinEventsConfirmedCVDwithCIMT_SA<-function(varString,exp,scen,nDigits=2){
  ...
  #print(length(idxMVvzv))
  #print(length(idxMVcmv))
  
  for(m in 1:max(tt$.imp)){
    if(scen=="worst"){
      if(exp=="vzv"){tt$vzvStatus_2levels[tt$.imp==m][idxMVvzv]<-"persistently high titres"}
      if(exp=="cmv"){tt$cmvStatus_2levels[tt$.imp==m][idxMVcmv]<-"persistently high titres"}
    }
    if(scen=="best"){
      if(exp=="vzv"){tt$vzvStatus_2levels[tt$.imp==m][idxMVvzv]<-"not persistently high titres"}
      if(exp=="cmv"){tt$cmvStatus_2levels[tt$.imp==m][idxMVcmv]<-"not persistently high titres"}
    }
  }
  tt<-as.mids(tt)
...
tab2.nocd4.sa.worst.vzv %>%
  kableRn(digits=3,caption="WORST case scenario: every missing VZV status = persistently high VZV. The association with VZV is remains statistically significant.") %>%
  kable_styling()
```
...
tab2.nocd4.sa.best.vzv %>%
  kableRn(digits=3,caption="BEST case scenario: every missing VZV status = persistently low VZV. The association with VZV remains statistically significant - in fact becomes slightly stronger.") %>%
  kable_styling() 
```
...
tab2.nocd4.sa.worst.cmv %>%
  kableRn(digits=3,caption="WORST case scenario: every missing CMV status = persistently high CMV. The association with CMV remains insignificant (and note that the association with VZV remains largely unchanged).") %>%
  kable_styling()
```
...
tab2.nocd4.sa.best.cmv %>%
  kableRn(digits=3,caption="BEST case scenario: every missing CMV status = persistently low CMV. The association with CMV remains insignificant (and note that the association with VZV remains largely unchanged).") %>%
  kable_styling()
```
...
ttCount %>%
  kable(caption="Listing of systemic disease event types. The numbers in brackets is the rate of events per 1000 person-years.", format = "html", escape = FALSE) %>%
  kable_styling(full_width = F)
```
...
summary(censModSystAFT)$summaryParameters %>%
  ...
  dplyr::select(!rownum) %>%
  kableRn(row_names = c("log Weibull shape parameter","log Weibull scale parameter","Participant type: PLWH","Sex: male","Age (years)","Waist-hip ratio (low)","Waist-hip ratio (high)"),caption="Results from an AFT model for clinical systemic events only.") %>%
  kable_styling(full_width = F)
```
...
modOuts %>%
  ...
  dplyr::select(coefficient,estimate,std.error,statistic,df,p.value) %>%
  mutate(p.value=ifelse(coefficient!="Intercept" & p.value<0.05,cell_spec(format(nsmall=4,round(digits=4,p.value)),bold=T),cell_spec(format(nsmall=4,round(digits=4,p.value)),bold=F))) %>%
  kableRn(row_names=NULL,row.names=F,caption=paste(sep="","Mediation analysis for the effect of VZV status through baseline inflammatory markers on clinical and sub-clinical (CIMT) CVD events. The 95% confidence interval for ACME is given by: (",paste(collapse=", ",format(nsmall=3,round(digits=3,acmeCI))),"). The 95% CI for the 3 natural indirect effects of ICAM-1, CD163 and IL-6 are given by (",paste(collapse=", ",format(nsmall=3,round(digits=3,indir1CI))),"), (",paste(collapse=", ",format(nsmall=3,round(digits=3,indir2CI))),") and (",paste(collapse=", ",format(nsmall=3,round(digits=3,indir3CI))),").")) %>%
  kable_styling(full_width = FALSE) %>%
...
cimtRelChangeSummary %>%
  kable(row.names=F,col.names=c("Participant type","VZV status","Minimum","Maximum","Median","25th percentile","75th percentile"), format = "html", escape = FALSE) %>%
  kable_styling(full_width = FALSE) %>%
...
# Sensitivty analysis comparing participants with and without recorded VZV, respectively CMV, data
...
rhiccaDatWide<-rhiccaDatWide %>%
  dplyr::mutate(
    vzvMissing=ifelse(is.na(vzvStatus_2levels),TRUE,FALSE),
    cmvMissing=ifelse(is.na(cmvStatus_2levels),TRUE,FALSE),
    vzvOrCmvMissing=ifelse(is.na(vzvStatus_2levels) | is.na(cmvStatus_2levels),TRUE,FALSE),
    completeFollowUp=ifelse(ltfu_24months==0,TRUE,FALSE)
  )

modMissVzv<-glm(vzvMissing~crf.age_group+crf.gender+crf.ptype+crf.smoke+crf.alcohol+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Cholesterol_enrolment+lims.Glucose_enrolment,data=rhiccaDatWide)

#summary(modMissVzv)

modMissCmv<-glm(cmvMissing~crf.age_group+crf.gender+crf.ptype+crf.smoke+crf.alcohol+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Cholesterol_enrolment+lims.Glucose_enrolment,data=rhiccaDatWide)

#summary(modMissCmv)

modMissVzvCmv<-glm(vzvOrCmvMissing~crf.age_group+crf.gender+crf.ptype+crf.smoke+crf.alcohol+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Cholesterol_enrolment+lims.Glucose_enrolment,data=rhiccaDatWide)

summary(modMissVzvCmv)

tt<-CreateTableOne(vars=c("crf.age","crf.gender","crf.ptype","crf.smoke","crf.alcohol","physio.bmi_enrolment","physio.bp_dia_enrolment","physio.bp_sys_enrolment","lims.Cholesterol_enrolment","lims.Glucose_enrolment","lims.CD4_Absolute_enrolment","lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment","cimt.cimt_enrolment","pwv.pwv_enrolment","completeFollowUp"),strata="vzvOrCmvMissing",data=rhiccaDatWide)

print(tt,nonnormal=c("crf.age","physio.bmi_enrolment","physio.bp_dia_enrolment","physio.bp_sys_enrolment","lims.Cholesterol_enrolment","lims.Glucose_enrolment","lims.CD4_Absolute_enrolment","lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment","cimt.cimt_enrolment","pwv.pwv_enrolment"),exact=c("crf.gender","crf.ptype","crf.smoke","crf.alcohol","completeFollowUp"))

out <- print(tt, noSpaces = TRUE, printToggle = FALSE, nonnormal=c("crf.age","physio.bmi_enrolment","physio.bp_dia_enrolment","physio.bp_sys_enrolment","lims.Cholesterol_enrolment","lims.Glucose_enrolment","lims.CD4_Absolute_enrolment","lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment","cimt.cimt_enrolment","pwv.pwv_enrolment"),exact=c("crf.gender","crf.ptype","crf.smoke","crf.alcohol","completeFollowUp"))

write.csv(out, file=paste(sep="","../output/",gsub(pattern="-",replacement="",Sys.Date()),"/RHICCA_",gsub(pattern="-",replacement="",Sys.Date()),"_comparisonMissingNonMissingVzvCmv.csv"))
```

# Stratified analysis by CD4 restoration and viral load suppression

## CD4 restoration

```{r}
rhiccaDatWide.orig<-rhiccaDatWide

rhiccaDatWide<-rhiccaDatWide.orig %>% dplyr::filter(!is.na(cd4Restoration) & cd4Restoration=="Full immune restoration")
tab2.cimtonly.cd4restored.cc<-funFitAndSum_modClinEventsCIMTonly.cc("crf.gender+crf.age+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months+log10(lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment)")
tab2.cimtonly.cd4restored.cc$term<-rownames(tab2.cimtonly.cd4restored.cc)

tab2.cimtonly.cd4restored.cc %>%
  kable(format="html", escape = FALSE, caption=paste(sep="","Sensitivity analysis for only those individuals with full CD4 count restoration, n=",nrow(rhiccaDatWide),".")) %>%
  kable_styling()
```

```{r}
rhiccaDatWide<-rhiccaDatWide.orig %>% dplyr::filter(!is.na(cd4Restoration) & cd4Restoration!="Full immune restoration")
tab2.cimtonly.cd4notrestored.cc<-funFitAndSum_modClinEventsCIMTonly.cc("crf.gender+crf.age+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months+log10(lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment)")
tab2.cimtonly.cd4notrestored.cc$term<-rownames(tab2.cimtonly.cd4notrestored.cc)

tab2.cimtonly.cd4notrestored.cc %>%
  kable(format="html", escape = FALSE, caption=paste(sep="","Sensitivity analysis for only those individuals without full CD4 count restoration, n=",nrow(rhiccaDatWide),".")) %>%
  kable_styling()

rhiccaDatWide<-rhiccaDatWide.orig
```

## VL suppression

```{r}
rhiccaDatWide<-rhiccaDatWide.orig %>% dplyr::filter(!is.na(vlSuppression) & vlSuppression=="Complete virological suppression")
tab2.cimtonly.vlSuppressed.cc<-funFitAndSum_modClinEventsCIMTonly.cc("crf.gender+crf.age+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months+log10(lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment)")
tab2.cimtonly.vlSuppressed.cc$term<-rownames(tab2.cimtonly.vlSuppressed.cc)

tab2.cimtonly.vlSuppressed.cc %>%
  kable(format="html", escape = FALSE, caption=paste(sep="","Sensitivity analysis for only those individuals with complete viral suppression, n=",nrow(rhiccaDatWide),".")) %>%
  kable_styling()
```

```{r}
rhiccaDatWide<-rhiccaDatWide.orig %>% dplyr::filter(!is.na(vlSuppression) & vlSuppression!="Complete virological suppression")
tab2.cimtonly.vlNotSuppressed.cc<-funFitAndSum_modClinEventsCIMTonly.cc("crf.gender+crf.age+physio.bmi_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months+log10(lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment)")
tab2.cimtonly.vlNotSuppressed.cc$term<-rownames(tab2.cimtonly.vlNotSuppressed.cc)

tab2.cimtonly.vlNotSuppressed.cc %>%
  kable(format="html", escape = FALSE, caption=paste(sep="","Sensitivity analysis for only those individuals without complete viral suppression, n=",nrow(rhiccaDatWide),".")) %>%
  kable_styling()

rhiccaDatWide<-rhiccaDatWide.orig
```

# Events per VZV exposure status depending on thresholds for CIMT events used...

```{r}
rhiccaDatWidePLWH<-rhiccaDatWide %>% dplyr::filter(crf.ptype=="Patient")

qAll<-quantile(prob=0.75,c(rhiccaDatWide$vzv.vzv_enrolment,rhiccaDatWide$vzv.vzv_6months),na.rm=T)
print(qAll)
qPLWH<-quantile(prob=0.75,c(rhiccaDatWidePLWH$vzv.vzv_enrolment,rhiccaDatWidePLWH$vzv.vzv_6months),na.rm=T)
print(qPLWH)

rhiccaDatWidePLWH<-rhiccaDatWidePLWH %>% dplyr::mutate(
  vzvStatusNew=factor(case_when(
    !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment<qPLWH & vzv.vzv_6months<qPLWH ~ "persistently low titres",
    !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment>=qPLWH & vzv.vzv_6months<qPLWH ~ "progression to low titres",
    !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment<qPLWH & vzv.vzv_6months>=qPLWH ~ "progression to high titres",
    !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment>=qPLWH & vzv.vzv_6months>=qPLWH ~ "persistently high titres",
    TRUE~NA_character_
  ),levels=c("persistently low titres","progression to low titres","progression to high titres","persistently high titres"))
) %>% dplyr::mutate(
  vzvStatusNew_2levels=factor(case_when(
    !is.na(vzvStatusNew) & vzvStatusNew=="persistently high titres"~"persistently high titres",
    !is.na(vzvStatusNew) & vzvStatusNew!="persistently high titres"~"not persistently high titres"
  ),levels=c("not persistently high titres","persistently high titres"))
)

table(rhiccaDatWidePLWH$numClinEventsPerParticipantCIMTonly,rhiccaDatWidePLWH$vzvStatusQuint_2levels)

table(rhiccaDatWidePLWH$numClinEventsPerParticipantCIMTonly,rhiccaDatWidePLWH$vzvStatus_2levels)

table(rhiccaDatWidePLWH$numClinEventsPerParticipantCIMTonly,rhiccaDatWidePLWH$vzvStatusNew_2levels)
```
  
# Clinical events in PLWH supplementary table numbers

## Total observation times in 1) persistently high VZV, 2) not persistently high VZV groups

The text says 'days' but as we divided by 365, these are years.

```{r}
sum(rhiccaDatWide$obsTime[rhiccaDatWide$crf.ptype=="Patient" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels=="persistently high titres"])/365

sum(rhiccaDatWide$obsTime[rhiccaDatWide$crf.ptype=="Patient" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels!="persistently high titres"])/365
```

## Preparing data with specific events for table

```{r}
pidStroke<-clinEventsDat$PID[grepl(pattern="stroke",tolower(clinEventsDat$DIAGNOSIS))]
pidMI<-rhiccaDatWide$pid[which(rhiccaDatWide$newOnsetMyocardialInfarction=="new MI")]
pidIschaemic<-clinEventsDat$PID[grepl(pattern="isch",tolower(clinEventsDat$DIAGNOSIS))]
pidPeripheral<-clinEventsDatFull$PID[grepl(pattern="peripheral",tolower(clinEventsDatFull$DIAGNOSIS))] # note that thie only event here has degree of certainty listed as 'not an event'; we previously filtered this out...
pidVascDeath<-clinEventsDat$PID[clinEventsDat$DIAGNOSIS=="death" & grepl(pattern="CVD",clinEventsDat$category)]

rhiccaDatWidePLWH<-rhiccaDatWide %>% dplyr::filter(crf.ptype=="Patient")
```

### Stroke events per VZV group:

```{r}
table(rhiccaDatWidePLWH$vzvStatus_2levels[rhiccaDatWidePLWH$pid %in% pidStroke],useNA="ifany")
```

### MI events per VZV group:

```{r}
table(rhiccaDatWidePLWH$vzvStatus_2levels[rhiccaDatWidePLWH$pid %in% pidMI],useNA="ifany")
```

### Ischaemic heart disease events per VZV group:

```{r}
table(rhiccaDatWidePLWH$vzvStatus_2levels[rhiccaDatWidePLWH$pid %in% pidIschaemic],useNA="ifany")
```

### Peripheral heart disease events per VZV group:

```{r}
table(rhiccaDatWidePLWH$vzvStatus_2levels[rhiccaDatWidePLWH$pid %in% pidPeripheral],useNA="ifany")
```

### CVD linked death per VZV group:

```{r}
table(rhiccaDatWidePLWH$vzvStatus_2levels[rhiccaDatWidePLWH$pid %in% pidVascDeath],useNA="ifany")
```
