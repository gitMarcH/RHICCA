...
rhiccaDatLong <- rhiccaDatLong %>%
  mutate(eGFR=egfrCalc(creatMmolL=lims.Creatinine,sex=crf.gender,age=crf.age))
...
rhiccaDatWide<-rhiccaDatWide %>%
  mutate(
    ...
    newOnsetHypercholesterolemia=case_when(
      lims.Cholesterol_enrolment>=5.2~"pre-exisiting hypercholesterolemia",
    ...
    newOnsetABPI=case_when(
      abpi.abpi_enrolment<=0.9~"pre-exisiting ABPI vasculopathy",
...
cmvDatWide<-cmvDatWide %>%
  dplyr::mutate(
    cmvStatus=factor(case_when(
      !is.na(cmv.cmv_enrolment) & !is.na(cmv.cmv_6months) & cmv.cmv_enrolment<cmvQuartiles["75%"] & cmv.cmv_6months<cmvQuartiles["75%"] ~ "persistently low CMV titres",
      !is.na(cmv.cmv_enrolment) & !is.na(cmv.cmv_6months) & cmv.cmv_enrolment>=cmvQuartiles["75%"] & cmv.cmv_6months<cmvQuartiles["75%"] ~ "progression to latent CMV",
      !is.na(cmv.cmv_enrolment) & !is.na(cmv.cmv_6months) & cmv.cmv_enrolment<cmvQuartiles["75%"] & cmv.cmv_6months>=cmvQuartiles["75%"] ~ "reactivated latent CMV",
      !is.na(cmv.cmv_enrolment) & !is.na(cmv.cmv_6months) & cmv.cmv_enrolment>=cmvQuartiles["75%"] & cmv.cmv_6months>=cmvQuartiles["75%"] ~ "persistently high CMV titres",
      TRUE~NA_character_
    ),levels=c("persistently low CMV titres","progress to latent CMV","reactivated latent CMV","persistently high CMV titres"))
  ) %>%
  mutate(
        cmvStatus_2levels=factor(case_when(
          !is.na(cmvStatus) & cmvStatus=="persistently high CMV titres"~"persistently high CMV titres",
          !is.na(cmvStatus) & cmvStatus!="persistently high CMV titres"~"not persistently high CMV titres"
        ),levels=c("not persistently high CMV titres","persistently high CMV titres"))
  )

vzvDatWide<-vzvDatWide %>%
  dplyr::mutate(
    vzvStatus=factor(case_when(
      !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment<vzvQuartiles["75%"] & vzv.vzv_6months<vzvQuartiles["75%"] ~ "persistently low VZV titres",
      !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment>=vzvQuartiles["75%"] & vzv.vzv_6months<vzvQuartiles["75%"] ~ "progression to latent VZV",
      !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment<vzvQuartiles["75%"] & vzv.vzv_6months>=vzvQuartiles["75%"] ~ "reactivated latent VZV",
      !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment>=vzvQuartiles["75%"] & vzv.vzv_6months>=vzvQuartiles["75%"] ~ "persistently high VZV titres",
      TRUE~NA_character_
    ),levels=c("persistently low VZV titres","progression to latent VZV","reactivated latent VZV","persistently high VZV titres"))
  ) %>%
  mutate(
        vzvStatus_2levels=factor(case_when(
          !is.na(vzvStatus) & vzvStatus=="persistently high VZV titres"~"persistently high VZV titres",
          !is.na(vzvStatus) & vzvStatus!="persistently high VZV titres"~"not persistently high VZV titres"
        ),levels=c("not persistently high VZV titres","persistently high VZV titres"))
  )
...
sumFun<-function(.data){
  ...
  dplyr::summarise(n=n(),
            ...
            cmvStatusPLow=sum(na.rm=TRUE,cmvStatus=="persistently low CMV titres"),
            cmvStatusPLowProp=sum(na.rm=TRUE,cmvStatus=="persistently low CMV titres")/sum(!is.na(cmvStatus)),
            cmvStatusPLowMV=NA,
            cmvStatusPLat=sum(na.rm=TRUE,cmvStatus=="progress to latent CMV"),
            cmvStatusPLatProp=sum(na.rm=TRUE,cmvStatus=="progress to latent CMV")/sum(!is.na(cmvStatus)),
            cmvStatusPLatMV=NA,
            cmvStatusRLat=sum(na.rm=TRUE,cmvStatus=="reactivated latent CMV"),
            cmvStatusRLatProp=sum(na.rm=TRUE,cmvStatus=="reactivated latent CMV")/sum(!is.na(cmvStatus)),
            cmvStatusRLatMV=NA,
            cmvStatusPHigh=sum(na.rm=TRUE,cmvStatus=="persistently high CMV titres"),
            cmvStatusPHighProp=sum(na.rm=TRUE,cmvStatus=="persistently high CMV titres")/sum(!is.na(cmvStatus)),
            cmvStatusPHighMV=NA,
            ...
            vzvStatusPLow=sum(na.rm=TRUE,vzvStatus=="persistently low VZV titres"),
            vzvStatusPLowProp=sum(na.rm=TRUE,vzvStatus=="persistently low VZV titres")/sum(!is.na(vzvStatus)),
            vzvStatusPLowMV=NA,
            vzvStatusPLat=sum(na.rm=TRUE,vzvStatus=="progress to latent VZV"),
            vzvStatusPLatProp=sum(na.rm=TRUE,vzvStatus=="progress to latent VZV")/sum(!is.na(vzvStatus)),
            vzvStatusPLatMV=NA,
            vzvStatusRLat=sum(na.rm=TRUE,vzvStatus=="reactivated latent VZV"),
            vzvStatusRLatProp=sum(na.rm=TRUE,vzvStatus=="reactivated latent VZV")/sum(!is.na(cmvStatus)),
            vzvStatusRLatMV=NA,
            vzvStatusPHigh=sum(na.rm=TRUE,vzvStatus=="persistently high VZV titres"),
            vzvStatusPHighProp=sum(na.rm=TRUE,vzvStatus=="persistently high VZV titres")/sum(!is.na(vzvStatus)),
            vzvStatusPHighMV=NA,
...
reformatFun<-function(dat,var,type,rowIdx,roundDigits=NULL){
  ...
  if(type=="medianIQR"){
    if(is.null(roundDigits)){roundDigits=3}
...
demoDatKable<-data.frame(
  variable=c("Participants","Followed-up at 24 months","Sex: male","Age","Age group [35,45)","Age group [45,55)","Age group [55,65)","Age group 65+","BMI (kg/m^2^)","Waist-hip ratio","Diastolic blood pressure (mm Hg)","Systolic blood pressure (mm Hg)","Blood pressure: normotensive","Blood pressure: pre-hypertensive","Blood pressure: hypertensive","Cholesterol (mmol/l)","Hypercholesteremic","Glucose (mmol/l)","Diabetic","eGFR (ml/min/1.73m^2^)","Chronic kidney disease","Current smoker","Framingham 10-year CVD risk","HIV infected","HIV viral load (copies/ml)","HIV CD4+ (cells/mm^3^)","CD163 (ng/ml)","ICAM-1 (pg/ml)","IL-6 (pg/ml)","IL-1b (pg/ml)","CMV (AU/ml)","CMV status","CMV status: persistently low titres","CMV status: progress to latent CMV","CMV status: reactivated latent CMV","CMV status: persistently high titres","VZV (mIU/ml)","VZV status","VZV status: persistently low titres","VZV status: progress to latent VZV","VZV status: reactivated latent VZV","VZV status: persistently high titres","PWV (m/s)","CIMT (mm)","ABPI","Confirmed clinical CVD events"),
  statistics=c("n (% out of total)","n (%)","n (%)","median (IQR)","n (%)","n (%)","n (%)","n (%)","median (IQR)","median (IQR)","mean (SD)","mean (SD)","n (%)","n (%)","n (%)","median (IQR)","n (%)","mean (SD)","n (%)","mean (SD)","n (%)","n (%)","median (IQR)","n (%)","median (IQR)","median (IQR)","median (IQR)","median (IQR)","median (IQR)","median (IQR)","median (IQR)","","n (%)","n (%)","n (%)","n (%)","median (IQR)","","n (%)","n (%)","n (%)","n (%)","mean (SD)","mean (SD)","mean (SD)","n (events per 1,000 person-years)"),
  AMV=NA,
  Astat1=NA,
  Astat2=NA,
  PMV=NA,
...
demoDatKable[demoDatKable$variable=="Framingham 10-year CVD risk",-c(1:2)]<-c(reformatFun(dat=demoDatSummary,var="framHamScore",type="medianIQR",rowIdx=1),reformatFun(dat=demoDatSummary,var="framHamScore",type="medianIQR",rowIdx=3),reformatFun(dat=demoDatSummary,var="framHamScore",type="medianIQR",rowIdx=2))
...
demoDatKable[demoDatKable$variable=="HIV viral load (copies/ml)",-c(1:2)]<-c(reformatFun(dat=demoDatSummary,var="HIVvl",type="medianIQR",rowIdx=1,roundDigits=0),reformatFun(dat=demoDatSummary,var="HIVvl",type="medianIQR",rowIdx=3,roundDigits=0),reformatFun(dat=demoDatSummary,var="HIVvl",type="medianIQR",rowIdx=2,roundDigits=0))
demoDatKable[demoDatKable$variable=="HIV CD4+ (cells/mm^3^)",-c(1:2)]<-c(reformatFun(dat=demoDatSummary,var="CD4",type="medianIQR",rowIdx=1,roundDigits=0),reformatFun(dat=demoDatSummary,var="CD4",type="medianIQR",rowIdx=3,roundDigits=0),reformatFun(dat=demoDatSummary,var="CD4",type="medianIQR",rowIdx=2,roundDigits=0))
demoDatKable[demoDatKable$variable=="CD163 (ng/ml)",-c(1:2)]<-c(reformatFun(dat=demoDatSummary,var="CD163",type="medianIQR",rowIdx=1),reformatFun(dat=demoDatSummary,var="CD163",type="medianIQR",rowIdx=3),reformatFun(dat=demoDatSummary,var="CD163",type="medianIQR",rowIdx=2))
demoDatKable[demoDatKable$variable=="ICAM-1 (pg/ml)",-c(1:2)]<-c(reformatFun(dat=demoDatSummary,var="ICAM1",type="medianIQR",rowIdx=1),reformatFun(dat=demoDatSummary,var="ICAM1",type="medianIQR",rowIdx=3),reformatFun(dat=demoDatSummary,var="ICAM1",type="medianIQR",rowIdx=2))
demoDatKable[demoDatKable$variable=="IL-6 (pg/ml)",-c(1:2)]<-c(reformatFun(dat=demoDatSummary,var="IL6",type="medianIQR",rowIdx=1),reformatFun(dat=demoDatSummary,var="IL6",type="medianIQR",rowIdx=3),reformatFun(dat=demoDatSummary,var="IL6",type="medianIQR",rowIdx=2))
demoDatKable[demoDatKable$variable=="IL-1b (pg/ml)",-c(1:2)]<-c(reformatFun(dat=demoDatSummary,var="IL1b",type="medianIQR",rowIdx=1),reformatFun(dat=demoDatSummary,var="IL1b",type="medianIQR",rowIdx=3),reformatFun(dat=demoDatSummary,var="IL1b",type="medianIQR",rowIdx=2))
...
demoDatKable[demoDatKable$variable=="CMV status: progress to latent CMV",-c(1:2)]<-c(reformatFun(dat=demoDatSummary,var="cmvStatusPLat",type="nProp",rowIdx=1),reformatFun(dat=demoDatSummary,var="cmvStatusPLat",type="nProp",rowIdx=3),reformatFun(dat=demoDatSummary,var="cmvStatusPLat",type="nProp",rowIdx=2))
demoDatKable[demoDatKable$variable=="CMV status: reactivated latent CMV",-c(1:2)]<-c(reformatFun(dat=demoDatSummary,var="cmvStatusRLat",type="nProp",rowIdx=1),reformatFun(dat=demoDatSummary,var="cmvStatusRLat",type="nProp",rowIdx=3),reformatFun(dat=demoDatSummary,var="cmvStatusRLat",type="nProp",rowIdx=2))
...
demoDatKable[demoDatKable$variable=="VZV status: progress to latent VZV",-c(1:2)]<-c(reformatFun(dat=demoDatSummary,var="vzvStatusPLat",type="nProp",rowIdx=1),reformatFun(dat=demoDatSummary,var="vzvStatusPLat",type="nProp",rowIdx=3),reformatFun(dat=demoDatSummary,var="vzvStatusPLat",type="nProp",rowIdx=2))
demoDatKable[demoDatKable$variable=="VZV status: reactivated latent VZV",-c(1:2)]<-c(reformatFun(dat=demoDatSummary,var="vzvStatusRLat",type="nProp",rowIdx=1),reformatFun(dat=demoDatSummary,var="vzvStatusRLat",type="nProp",rowIdx=3),reformatFun(dat=demoDatSummary,var="vzvStatusRLat",type="nProp",rowIdx=2))
...
demoDatKable[demoDatKable$variable=="Confirmed clinical CVD events",-c(1:2)]<-c(demoDatSummary[1,"clinEventsConfirmedCVD"],format(nsmall=2,round(demoDatSummary[1,"clinEventsConfirmedCVDPer1000PYs"],digits=3)),demoDatSummary[1,"clinEventsConfirmedCVDMV"],demoDatSummary[3,"clinEventsConfirmedCVD"],format(nsmall=2,round(demoDatSummary[3,"clinEventsConfirmedCVDPer1000PYs"],digits=3)),demoDatSummary[3,"clinEventsConfirmedCVDMV"],demoDatSummary[2,"clinEventsConfirmedCVD"],format(nsmall=2,round(demoDatSummary[2,"clinEventsConfirmedCVDPer1000PYs"],digits=3)),demoDatSummary[2,"clinEventsConfirmedCVDMV"])
...
demoDatKable %>%
  ...
  pack_rows("Modifiable risk factors",9,22) %>%
  pack_rows("Framingham risk score",23,23) %>%
  pack_rows("Exposure variables",24,26) %>%
  pack_rows("Mediators",27,30) %>%
  pack_rows("Mediators - herpes",31,42) %>%
  pack_rows("Outcomes",43,46)

write.csv(demoDatKable,row.names=F,file=paste(sep="_",outPrefix,"participantCharacteristicsTable.csv"))
```
...
* **Progression to latency**: titres at enrolment in the top quartile of values and titres at 6 months in bottom 3 quartiles
* **Reactivated latent infection**: titres at enrolment in the bottom 3 quartiles of values and titres at 6 months in the top quartile
...
impDatLong<-impDat %>%
  ...
  pivot_longer(cols=!c(pid,starts_with("crf"),contains("waisthip"),lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment,contains("Glucose"),contains("Cholesterol"),lims.CD4_Absolute_enrolment,iris),names_to=c(".value","visit"),names_pattern="([a-zA-z0-9_\\.]+)_(enrolment|[0-9]+months)") %>%
...
if(!skip.Imp){
  ...
  for(i in 1:m){
    ...
    tmpLong<-pivot_longer(tmp,cols=!c(pid,starts_with("crf"),contains("waisthip"),lims.HIV_Abbott_Viral_Load_0pt2ml_P_enrolment,contains("Glucose"),contains("Cholesterol"),lims.CD4_Absolute_enrolment,iris),names_to=c(".value","visit"),names_pattern="([a-zA-z0-9_\\.]+)_(enrolment|[0-9]+months)")
  ...
  tmpLong<-mice::complete(rhiccaDatWide_impMice, action='long', include=TRUE) %>%
    mutate(
      ...
      !is.na(cmv.cmv_enrolment) & !is.na(cmv.cmv_6months) & cmv.cmv_enrolment>=cmvQuartiles["75%"] & cmv.cmv_6months<cmvQuartiles["75%"] ~ "progression to latent CMV",
        ...
        !is.na(cmv.cmv_enrolment) & !is.na(cmv.cmv_6months) & cmv.cmv_enrolment>=cmvQuartiles["75%"] & cmv.cmv_6months>=cmvQuartiles["75%"] ~ "persistently high CMV titres",
        TRUE~NA_character_
      ),levels=c("persistently low CMV titres","progression to latent CMV","reactivated latent CMV","persistently high CMV titres")),
      vzvStatus=factor(case_when(
        !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment<vzvQuartiles["75%"] & vzv.vzv_6months<vzvQuartiles["75%"] ~ "persistently low VZV titres",
        !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment>=vzvQuartiles["75%"] & vzv.vzv_6months<vzvQuartiles["75%"] ~ "progression to latent VZV",
        !is.na(vzv.vzv_enrolment) & !is.na(vzv.vzv_6months) & vzv.vzv_enrolment<vzvQuartiles["75%"] & vzv.vzv_6months>=vzvQuartiles["75%"] ~ "reactivated latent VZV",
...
ggModRiskFactors<-datTmp %>%
  ...
  ylab("") +
  xlim(c(0,24)) +
  facet_wrap(~factor(var,levels=c("CD4+","HIV viral load","BMI","CMV","Waist-to-hip ratio","VZV","Bp diastolic","CD163","Bp systolic","ICAM-1","Cholesterol","IL-6","Glucose")),scales="free_y",nrow=7,ncol=2)

print(ggModRiskFactors)

pdf(width=12,height=12,file=paste(sep="_",outPrefix,"CIMT_CVDevents_Figure2.pdf"))
print(ggModRiskFactors)
dev.off()
```
...
geeModCimt<-geeglm(cimt.cimt~physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment,data=tmpDatAllAdj,id=factor(tmpDatAllAdj$pid),corstr="exchangeable")
...
obj1GlmCimtAdjGlucChol<-glm(cimtChange~ifelse(crf.ptype=="Patient",1,0)+cimt.cimt_enrolment+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment,data=tmpDatWide) # another marker of vasculopathy; unadjusted model
obj1GlmCimtAdjGlucCholInt<-glm(cimtChange~crf.ptype+cimt.cimt_enrolment+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment*crf.gender+lims.Cholesterol_enrolment*crf.gender,data=tmpDatWide) # another marker of vasculopathy; unadjusted model

obj1GlmCimtAdjGlucChol.mid<-with(rhiccaDatWide_impMice,glm(cimtChange~crf.ptype+cimt.cimt_enrolment+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus+vzvStatus)) 
#summary(pool(obj1GlmCimtAdjGlucChol.mid))

obj1GlmCimtAdjGlucChol.quantCmvVzv.mid<-with(rhiccaDatWide_impMice,glm(cimtChange~ifelse(crf.ptype=="Patient",1,0)+cimt.cimt_enrolment+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmv.cmv_enrolment+cmv.cmv_CFB+vzv.vzv_enrolment+vzv.vzv_CFB)) # no sig assoc with VZV
...
obj1GlmCimtAdjGlucCholSolMarkers.mid<-with(rhiccaDatWide_impMice,glm(cimtChange~ifelse(crf.ptype=="Patient",1,0)+cimt.cimt_enrolment+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+lum.ICAM1_enrolment+lum.CD163_enrolment+lum.IL6_enrolment)) 

obj1GlmCimtAdjGlucChol.noPtype.mid<-with(rhiccaDatWide_impMice,glm(cimtChange~cimt.cimt_enrolment+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment)) 
...
obj1GlmCimtAdjGlucCholSolMarkers<-glm(cimtChange~ifelse(crf.ptype=="Patient",1,0)+cimt.cimt_enrolment+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment*crf.gender+lims.Cholesterol_enrolment*crf.gender+lum.CD163_enrolment+lum.ICAM1_enrolment,data=tmpDatWide) # another marker of vasculopathy; unadjusted model
...
obj1GlmCimtAdjGlucChol.hiv.mid<-with(rhiccaDatWide_impMice.hiv,glm(cimtChange~cimt.cimt_enrolment+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus+vzvStatus+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months))
obj1GlmCimtAdjGlucChol.quantCmvVzv.hiv.mid<-with(rhiccaDatWide_impMice.hiv,glm(cimtChange~cimt.cimt_enrolment+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmv.cmv_enrolment+cmv.cmv_CFB+vzv.vzv_enrolment+vzv.vzv_CFB+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months)) # no sig assoc with VZV
...
tab2<-sumFitandSum_cimt("+crf.ptype+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus+vzvStatus")

tab3<-sumFitandSum_cimt.hiv("+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus+vzvStatus+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months")
...
rn<-c("Baseline CIMT","Participant type: PLWH","Sex: male","Age (in years)","Waist-hip ratio","Diastolic blood pressure","Systolic blood pressure","Glucose","Cholesterol","CMV: progression to latent CMV","CMV: reactivated latent CMV","CMV: persistently high titres","VZV: progression to latent VZV","VZV: reactivated latent VZV","VZV: persistently high titres","CD4 (baseline)","CD4 (change from Baseline at 24 months)")

rnAlt<-c(
    c("Intercept ","Participant type: PLWH ","Baseline CIMT ","CMV: progression to latent CMV","CMV: reactivated latent CMV","CMV: persistently high titres","VZV: progression to latent VZV","VZV: reactivated latent VZV","VZV: persistently high titres"),
    c("Intercept  ","Baseline CIMT  ","CMV: progression to latent CMV ","CMV: reactivated latent CMV ","CMV: persistently high titres ","VZV: progression to latent VZV ","VZV: reactivated latent VZV ","VZV: persistently high titres ")
  )

rnAlt2<-c(
    c("Intercept ","Participant type: PLWH ","Baseline CIMT ","Sex: male","Age (in years)","Waist-hip ratio","Diastolic blood pressure","Systolic blood pressure","Glucose","Cholesterol","CMV (enrolment)","CMV (change from baseline at 6 months)","VZV (enrolment)","VZV (change from baseline at 6 months)"),
    c("Intercept  ","Baseline CIMT  ","Sex: male ","Age (in years) ","Waist-hip ratio ","Diastolic blood pressure ","Systolic blood pressure ","Glucose ","Cholesterol ","CMV (enrolment) ","CMV (change from baseline at 6 months) ","VZV (enrolment) ","VZV (change from baseline at 6 months) ","CD4 (baseline)","CD4 (change from Baseline at 24 months)")
  )
...
rn<-c("Baseline CIMT","Sex: male","Age (in years)","Waist-hip ratio","Diastolic blood pressure","Systolic blood pressure","Glucose","Cholesterol","CMV: progression to latent CMV","CMV: reactivated latent CMV","CMV: persistently high titres","VZV: progression to latent VZV","VZV: reactivated latent VZV","VZV: persistently high titres","CD4 (baseline)","CD4 (change from Baseline at 24 months)")
...
geeModPwv<-geeglm(pwv.pwv~physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment,data=tmpDatAllAdj,id=factor(tmpDatAllAdj$pid),corstr="exchangeable")
...
obj1LmmPwvAdjGlucChol<-lmerTest::lmer(pwv.pwv~visitNum*crf.ptype+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia+physio.bp_sys+lims.Glucose_enrolment+lims.Cholesterol_enrolment+(1|pid),data=rhiccaDatLong)
obj1LmmPwvAdjGlucChol.mid<-with(rhiccaDatLong_impMice,lme4::lmer(pwv.pwv~visitNum*crf.ptype+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia+physio.bp_sys+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus+vzvStatus+(1|pid)))
...
tab2<-sumFitandSum_pwv("visitNum*crf.ptype+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia+physio.bp_sys+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus+vzvStatus")

tab3<-sumFitandSum_pwv.hiv("visitNum+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia+physio.bp_sys+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus+vzvStatus+lims.CD4_Absolute_enrolment+lims.CD4_Absolute")
...
rn<-c("Time since baseline visit (in months)","Participant type: PLWH","Sex: male","Age (in years)","Waist-hip ratio","Diastolic blood pressure","Systolic blood pressure","Glucose","Cholesterol","CMV: progression to latent CMV","CMV: reactivated latent CMV","CMV: persistently high titres","VZV: progression to latent VZV","VZV: reactivated latent VZV","VZV: persistently high titres","Change in time since baseline coefficient in PLWH","CD4 (baseline)","CD4 (longitudinal)")
...
rn<-c("Time since baseline visit (in months)","Sex: male","Age (in years)","Waist-hip ratio","Diastolic blood pressure","Systolic blood pressure","Glucose (baseline)","Cholesterol (baseline)","CMV: progression to latent CMV","CMV: reactivated latent CMV","CMV: persistently high titres","VZV: progression to latent VZV","VZV: reactivated latent VZV","VZV: persistently high titres","CD4 (baseline)","CD4 (longitudinal)")
...
rn<-c("CIMT (baseline)","Time since baseline visit (in months)","Sex: male","Age (in years)","Waist-hip ratio","Diastolic blood pressure (baseline) *","Systolic blood pressure (baseline) *","Glucose (baseline)","Cholesterol (baseline)","CMV: progression to latent CMV","CMV: reactivated latent CMV","CMV: persistently high titres","VZV: progression to latent VZV","VZV: reactivated latent VZV","VZV: persistently high titres","CD4 (baseline)","CD4 (change from baseline) *")
...
for(j in which(!is.na(rhiccaDatWide$clinEventCategoryConfirmed) & rhiccaDatWide$clinEventCategoryConfirmed!="")){
  ...
  clinEventsTmp<-rbind(clinEventsTmp,data.frame(pid=rhiccaDatWide$pid[j],ptype=rhiccaDatWide$crf.PLWH[j],artnaive=rhiccaDatWide$crf.artnaive[j],artRegimen=rhiccaDatWide$crf.artRegimen.2levels[j],gender=rhiccaDatWide$crf.gender[j],age_group=rhiccaDatWide$crf.age_group[j],cmvStatus=rhiccaDatWide$cmvStatus[j],vzvStatus=rhiccaDatWide$vzvStatus[j],category=unlist(cats)))
}
...
tmpDeath<-rhiccaDatWide %>%
      ...
      dplyr::select(pid,crf.PLWH,crf.artnaive,crf.artRegimen.2levels,crf.gender,crf.age_group,cmvStatus,vzvStatus)
colnames(tmpDeath)<-c("pid","ptype","artnaive","artRegimen","gender","age_group","cmvStatus","vzvStatus")
...
tmpCimt<-rhiccaDatWide %>%
      ...
      dplyr::select(pid,crf.PLWH,crf.artnaive,crf.artRegimen.2levels,crf.gender,crf.age_group,cmvStatus,vzvStatus)
colnames(tmpCimt)<-c("pid","ptype","artnaive","artRegimen","gender","age_group","cmvStatus","vzvStatus")
...
clinEventsTmp<-cbind(
  ...
  c(clinEventsTmp[,ncol(clinEventsTmp)],rep("CIMT",sum(!is.na(rhiccaDatWide$newOnsetCIMT) & rhiccaDatWide$newOnsetCIMT=="new CIMT vasculopathy")))
)
...
tt<-tt[,c("Death","Cancer","Infection","Systemic disease","Undetermined","CVD","CIMT")]
tt<-cbind(rowSums(tt[,!(colnames(tt) %in% c("Death","CIMT"))]),tt)
colnames(tt)[1]<-"Any clinical event"
colnames(tt)[ncol(tt)]<-"CVD (sub-clinical, CIMT)"
...
clinEventsTmp<-clinEventsTmp %>%
  dplyr::filter(category %in% c("CVD","CIMT"))
...
clinEventsTmp2<-clinEventsTmp %>%
  ...
  dplyr::mutate(vzvStatus_2levels=ifelse(vzvStatus=="persistently high VZV titres","persistently high VZV titres","not persistently high VZV titres"))
...
modClinEventsConfirmedCVD.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVD~vzvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
modClinEventsCIMTonly.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCIMTonly~vzvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
# modClinEventsPWVonly.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantPWVonly~vzvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
modClinEventsConfirmedCVDwithCIMT.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMT~vzvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
modClinEventsConfirmedCVDwithCIMT.univariable.2levels<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMT~vzvStatus_2levels,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
# modClinEventsConfirmedCVDwithCIMTPWV.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMTPWV~vzvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))

cvdClinicalPoisP<-summary(pool(modClinEventsConfirmedCVD.univariable))$p.value[summary(pool(modClinEventsConfirmedCVD.univariable))$term=="vzvStatuspersistently high VZV titres"]

totCases<-sum(ttCountCount[,"CVD.clinical"])
incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="persistently high VZV titres","CVD.clinical"])/sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus!="persistently high VZV titres"])/(1000*365.25))
hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="persistently high VZV titres","CVD.clinical"])+incNonExp*sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus=="persistently high VZV titres"])/(1000*365.25))
pafClinical<-(totCases-hypoCases)/totCases

cvdCimtPoisP<-summary(pool(modClinEventsCIMTonly.univariable))$p.value[summary(pool(modClinEventsCIMTonly.univariable))$term=="vzvStatuspersistently high VZV titres"]

totCases<-sum(ttCountCount[,"CVD.CIMT"])
incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="persistently high VZV titres","CVD.CIMT"])/sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus!="persistently high VZV titres"])/(1000*365.25))
hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="persistently high VZV titres","CVD.CIMT"])+incNonExp*sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus=="persistently high VZV titres"])/(1000*365.25))
pafCimt<-(totCases-hypoCases)/totCases

# cvdPwvPoisP<-summary(pool(modClinEventsPWVonly.univariable))$p.value[summary(pool(modClinEventsPWVonly.univariable))$term=="vzvStatuspersistently high VZV titres"]
cvdClinicalCimtPoisP<-summary(pool(modClinEventsConfirmedCVDwithCIMT.univariable))$p.value[summary(pool(modClinEventsConfirmedCVDwithCIMT.univariable))$term=="vzvStatuspersistently high VZV titres"]
cvdClinicalCimtPoisP.2levels<-summary(pool(modClinEventsConfirmedCVDwithCIMT.univariable.2levels))$p.value[summary(pool(modClinEventsConfirmedCVDwithCIMT.univariable.2levels))$term=="vzvStatus_2levelspersistently high VZV titres"]

totCases<-sum(ttCountCount[,"CVD.clinicalAndCIMT"])
incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="persistently high VZV titres","CVD.clinicalAndCIMT"])/sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus!="persistently high VZV titres"])/(1000*365.25))
incExp<-sum(ttCountCount[rownames(ttCountCount)=="persistently high VZV titres","CVD.clinicalAndCIMT"])/sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus=="persistently high VZV titres"])/(1000*365.25))
...
hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="persistently high VZV titres","CVD.clinicalAndCIMT"])+incNonExp*sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus=="persistently high VZV titres"])/(1000*365.25))
pafClinCimt<-(totCases-hypoCases)/totCases

# cvdClinicalCimtPwvPoisP<-summary(pool(modClinEventsConfirmedCVDwithCIMTPWV.univariable))$p.value[summary(pool(modClinEventsConfirmedCVDwithCIMTPWV.univariable))$term=="vzvStatuspersistently high VZV titres"]
...
# kable(ttCount,caption="CVD clinical and sub-clinical events in participants, stratified by whether VZV status. The number in brackets is the rate of events per 1,000 person-years.",col.names=paste(sep=", Poisson regression p-value for persistently high VZV titres = ",c("Clinical events","CIMT sub-clinical events","PWV sub-clinical events","Clinical & CIMT sub-clinical events","Clinical & CIMT & PWV sub-clinical events"),format(nsmall=3,round(digits=3,c(cvdClinicalPoisP,cvdCimtPoisP,cvdPwvPoisP,cvdClinicalCimtPoisP,cvdClinicalCimtPwvPoisP))))) %>%
#   kable_styling(full_width = FALSE)
...
for(j in 1:ncol(ttCount)){
  for(i in 1:nrow(ttCount)){
    ttCount[i,j]<-paste(sep="",ttCount[i,j]," (",format(nsmall=3,round(digits=3,tt[i,j])),", 95% CI [",paste(collapse=", ",format(nsmall=3,round(digits=3,c(ttLow[i,j],ttUpp[i,j])))),"])")
    if(rownames(ttCount)[i]=="not persistently high VZV titres"){
    ...
    if(rownames(ttCount)[i]=="persistently high VZV titres"){
...
modClinEventsConfirmedCVD.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVD~cmvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
modClinEventsCIMTonly.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCIMTonly~cmvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
# modClinEventsPWVonly.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantPWVonly~cmvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
modClinEventsConfirmedCVDwithCIMT.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMT~cmvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
# modClinEventsConfirmedCVDwithCIMTPWV.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMTPWV~cmvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))

cvdClinicalPoisP<-summary(pool(modClinEventsConfirmedCVD.univariable))$p.value[summary(pool(modClinEventsConfirmedCVD.univariable))$term=="cmvStatuspersistently high CMV titres"]
cvdCimtPoisP<-summary(pool(modClinEventsCIMTonly.univariable))$p.value[summary(pool(modClinEventsCIMTonly.univariable))$term=="cmvStatuspersistently high CMV titres"]
# cvdPwvPoisP<-summary(pool(modClinEventsPWVonly.univariable))$p.value[summary(pool(modClinEventsPWVonly.univariable))$term=="cmvStatuspersistently high CMV titres"]
cvdClinicalCimtPoisP<-summary(pool(modClinEventsConfirmedCVDwithCIMT.univariable))$p.value[summary(pool(modClinEventsConfirmedCVDwithCIMT.univariable))$term=="cmvStatuspersistently high CMV titres"]
# cvdClinicalCimtPwvPoisP<-summary(pool(modClinEventsConfirmedCVDwithCIMTPWV.univariable))$p.value[summary(pool(modClinEventsConfirmedCVDwithCIMTPWV.univariable))$term=="cmvStatuspersistently high CMV titres"]

# kable(ttCount,caption="CVD clinical and sub-clinical events in participants, stratified by whether CMV status. The number in brackets is the rate of events per 1,000 person-years.",col.names=paste(sep=", Poisson regression p-value for persistently high CMV titres = ",c("Clinical events","CIMT sub-clinical events","PWV sub-clinical events","Clinical & CIMT sub-clinical events","Clinical & CIMT & PWV sub-clinical events"),format(nsmall=3,round(digits=3,c(cvdClinicalPoisP,cvdCimtPoisP,cvdPwvPoisP,cvdClinicalCimtPoisP,cvdClinicalCimtPwvPoisP))))) %>%
#   kable_styling(full_width = FALSE)
...
kable(ttCountAll,caption="CVD clinical and sub-clinical events in participants. The number in brackets is the rate of events per 1,000 person-years.",col.names=c("Clinical events","CIMT sub-clinical events","Clinical & CIMT sub-clinical events")) %>%
  kable_styling(full_width = FALSE) %>%
...
We now repeat this for only PLWH.
...
```{r}
tmp<-clinEventsTmpPLWH %>% dplyr::filter(!is.na(vzvStatus) & vzvStatus=="persistently high VZV titres")
...
for(i in 1:nrow(ttCount)){
  if(i==1){
    nEvents<-unlist(tt[i,])
    tObs<-sum(as.numeric(rhiccaDatWide$obsTime[rhiccaDatWide$crf.age<55 & rhiccaDatWide$crf.PLWH=="PLWH" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels=="persistently high VZV titres"])/(1000*365.25))
    
    tt[i,]<-nEvents/tObs
    ttLow[i,]<-(qchisq(p=0.025,df=2*nEvents)/2)/tObs
    ttUpp[i,]<-(qchisq(p=0.975,df=2*(nEvents+1))/2)/tObs
    # tt[i,]<-tt[i,]/sum(as.numeric(rhiccaDatWide$obsTime[rhiccaDatWide$crf.age<55 & rhiccaDatWide$crf.PLWH=="PLWH" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels=="persistently high VZV titres"])/(1000*365.25))
    # #nEvents<-
    # ttLow[i,]=tt[i,]-qnorm(0.975)*sqrt(log(tt[i,])/sum(as.numeric(rhiccaDatWide$obsTime[rhiccaDatWide$crf.age<55 & rhiccaDatWide$crf.PLWH=="PLWH" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels=="persistently high VZV titres"])/(1000*365.25)))
    # ttUpp[i,]=tt[i,]+qnorm(0.975)*sqrt(tt[i,]/sum(as.numeric(rhiccaDatWide$obsTime[rhiccaDatWide$crf.age<55 & rhiccaDatWide$crf.PLWH=="PLWH" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels=="persistently high VZV titres"])/(1000*365.25)))
  }else if(i==2){
    nEvents<-unlist(tt[i,])
    tObs<-sum(as.numeric(rhiccaDatWide$obsTime[rhiccaDatWide$crf.age>=55 & rhiccaDatWide$crf.PLWH=="PLWH" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels=="persistently high VZV titres"])/(1000*365.25))
    ...
    ttUpp[i,]<-(qchisq(p=0.975,df=2*(nEvents+1))/2)/tObs
    # tt[i,]<-tt[i,]/sum(as.numeric(rhiccaDatWide$obsTime[rhiccaDatWide$crf.age>=55 & rhiccaDatWide$crf.PLWH=="PLWH" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels=="persistently high VZV titres"])/(1000*365.25))
    # ttLow[i,]=tt[i,]-qnorm(0.975)*sqrt(tt[i,]/sum(as.numeric(rhiccaDatWide$obsTime[rhiccaDatWide$crf.age>=55 & rhiccaDatWide$crf.PLWH=="PLWH" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels=="persistently high VZV titres"])/(1000*365.25)))
    # ttUpp[i,]=tt[i,]+qnorm(0.975)*sqrt(tt[i,]/sum(as.numeric(rhiccaDatWide$obsTime[rhiccaDatWide$crf.age>=55 & rhiccaDatWide$crf.PLWH=="PLWH" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels=="persistently high VZV titres"])/(1000*365.25)))
  }
}
...
tmp<-clinEventsTmpPLWH %>%
  dplyr::filter(!is.na(vzvStatus) & vzvStatus=="persistently high VZV titres")
...
for(i in 1:nrow(ttCount)){
    nEvents<-unlist(tt[i,])
    tObs<-sum(as.numeric(rhiccaDatWide$obsTime[rhiccaDatWide$crf.gender==rownames(ttCount)[i] & rhiccaDatWide$crf.PLWH=="PLWH" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels=="persistently high VZV titres"])/(1000*365.25))
...
modClinEventsConfirmedCVD.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed %>% dplyr::filter(crf.ptype=="Patient"),glm(numClinEventsPerParticipantCVD~vzvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
modClinEventsCIMTonly.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed %>% dplyr::filter(crf.ptype=="Patient"),glm(numClinEventsPerParticipantCIMTonly~vzvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
modClinEventsConfirmedCVDwithCIMT.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed %>% dplyr::filter(crf.ptype=="Patient"),glm(numClinEventsPerParticipantCVDwithCIMT~vzvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
modClinEventsConfirmedCVDwithCIMT.univariable.2levels<-with(rhiccaDatWide_impMice.clinEventsConfirmed %>% dplyr::filter(crf.ptype=="Patient"),glm(numClinEventsPerParticipantCVDwithCIMT~vzvStatus_2levels,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
...
totCases<-sum(ttCountCount[,"CVD.CIMT"])
incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="persistently high VZV titres","CVD.CIMT"])/sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus!="persistently high VZV titres" & rhiccaDatWide$crf.PLWH=="PLWH"])/(1000*365.25))
hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="persistently high VZV titres","CVD.CIMT"])+incNonExp*sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus=="persistently high VZV titres" & rhiccaDatWide$crf.PLWH=="PLWH"])/(1000*365.25))
pafCimt<-(totCases-hypoCases)/totCases
...
totCases<-sum(ttCountCount[,"CVD.clinicalAndCIMT"])
incNonExp<-sum(ttCountCount[rownames(ttCountCount)!="persistently high VZV titres","CVD.clinicalAndCIMT"])/sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus!="persistently high VZV titres" & rhiccaDatWide$crf.PLWH=="PLWH"])/(1000*365.25))
incExp<-sum(ttCountCount[rownames(ttCountCount)=="persistently high VZV titres","CVD.clinicalAndCIMT"])/sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus=="persistently high VZV titres" & rhiccaDatWide$crf.PLWH=="PLWH"])/(1000*365.25))
...
hypoCases<-sum(ttCountCount[rownames(ttCountCount)!="persistently high VZV titres","CVD.clinicalAndCIMT"])+incNonExp*sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus=="persistently high VZV titres" & rhiccaDatWide$crf.PLWH=="PLWH"])/(1000*365.25))
pafClinCimt<-(totCases-hypoCases)/totCases
se<-sqrt(incNonExp/sum(rownames(ttCountCount)!="persistently high VZV titres"))
incNonExpL<-incNonExp-qnorm(0.975)*se
incNonExpU<-incNonExp+qnorm(0.975)*se
hypoCasesL<-sum(ttCountCount[rownames(ttCountCount)!="persistently high VZV titres","CVD.clinicalAndCIMT"])+incNonExpL*sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus=="persistently high VZV titres" & rhiccaDatWide$crf.PLWH=="PLWH"])/(1000*365.25))
hypoCasesU<-sum(ttCountCount[rownames(ttCountCount)!="persistently high VZV titres","CVD.clinicalAndCIMT"])+incNonExpU*sum(as.numeric(rhiccaDatWide$obsTime[!is.na(rhiccaDatWide$vzvStatus) & rhiccaDatWide$vzvStatus=="persistently high VZV titres" & rhiccaDatWide$crf.PLWH=="PLWH"])/(1000*365.25))
...
modClinEventsConfirmedCVD.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed %>% dplyr::filter(crf.ptype=="Patient"),glm(numClinEventsPerParticipantCVD~cmvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
modClinEventsCIMTonly.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed %>% dplyr::filter(crf.ptype=="Patient"),glm(numClinEventsPerParticipantCIMTonly~cmvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
modClinEventsConfirmedCVDwithCIMT.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed %>% dplyr::filter(crf.ptype=="Patient"),glm(numClinEventsPerParticipantCVDwithCIMT~cmvStatus,family="poisson",offset=log(obsTime/(1000*365.25)+1)))
...
kable(ttCountAll,caption="CVD clinical and sub-clinical events in PLWH. The number in brackets is the rate of events per 1,000 person-years.",col.names=c("Clinical events","CIMT sub-clinical events","Clinical & CIMT sub-clinical events")) %>%
  ...
  pack_rows(start_row=10,end_row=13,group_label="Stratified by CMV titres.")
```

The absolute risk (expressed as an interest rate) of clinical and sub-clinical CIMT events is `r airCvdpVZV` per 1,000 person-years **in PLWH** with pVZV compared to `r airCvdNotpVZV` per 1,000 person-years in those without pVZV (Poisson p-value for this difference p=`r format(nsmall=3,round(digits=3,cvdClinicalCimtPoisP.2levels))`).
...
tmp<-clinEventsTmpCtrl %>% dplyr::filter(!is.na(vzvStatus) & vzvStatus=="persistently high VZV titres")
tt<-table(tmp$age_group_2lvls,tmp$category)
ttt<-data.frame(
  CVD.clinical=tt[,"CVD"],
  CVD.CIMT=tt[,"CIMT"]
...
for(i in 1:nrow(ttCount)){
  if(i==1){
    nEvents<-unlist(tt[i,])
    tObs<-sum(as.numeric(rhiccaDatWide$obsTime[rhiccaDatWide$crf.age<55 & rhiccaDatWide$crf.PLWH=="Control" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels=="persistently high VZV titres"])/(1000*365.25))
...
for(i in 1:nrow(ttCount)){
    nEvents<-unlist(tt[i,])
    tObs<-sum(as.numeric(rhiccaDatWide$obsTime[rhiccaDatWide$crf.gender==rownames(ttCount)[i] & rhiccaDatWide$crf.PLWH=="Control" & !is.na(rhiccaDatWide$vzvStatus_2levels) & rhiccaDatWide$vzvStatus_2levels=="persistently high VZV titres"])/(1000*365.25))
...
```{r}
# modClinEvents<-with(rhiccaDatWide_impMice.clinEvents,glm(numClinEventsPerParticipant~crf.ptype*crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment,family="poisson"))
# 
# modClinEventsNoOnset<-with(rhiccaDatWide_impMice.clinEvents,glm(numClinEventsPerParticipantNoOnset~crf.ptype*crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+pwv.pwv_enrolment+pwvChange+cimt.cimt_enrolment+cimtChange,family="poisson"))
# 
# modClinEventsNoPwvCimtAbpi<-with(rhiccaDatWide_impMice.clinEvents,glm(numClinEventsPerParticipantNoPwvCimtAbpi~crf.ptype*crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+pwv.pwv_enrolment+pwvChange+cimt.cimt_enrolment+cimtChange+cmvStatus+vzvStatus,family="poisson"))
# 
...
# 
# modClinEventsConfirmedWithDerivedWithoutPwvCimtAbpi<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantWithDerivedWithoutPwvCimtAbpi~crf.ptype*crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+pwv.pwv_enrolment+pwvChange+cimt.cimt_enrolment+cimtChange,family="poisson"))
# 
# modClinEventsPwvCimtAbpi<-with(rhiccaDatWide_impMice.clinEvents,glm(numClinEventsPwvCimtAbpi~crf.ptype*crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmv.cmv_6months+vzv.vzv_6months,family="poisson"))
# 
# modClinEventsPwvCimt<-with(rhiccaDatWide_impMice.clinEvents,glm(numClinEventsPwvCimt~crf.ptype*crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus+vzvStatus,family="poisson"))
# 
# modClinEventsCVD<-with(rhiccaDatWide_impMice.clinEvents,glm(numClinEventsPerParticipantCVD~crf.ptype+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+lum.CD163_enrolment+lum.ICAM1_enrolment+pwv.pwv_enrolment+pwvChange+cimt.cimt_enrolment+cimtChange,family="poisson"))
# 
# modClinEventsConfirmedCVD<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVD~crf.ptype*crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus+vzvStatus,family="poisson"))
# 
# modClinEventsConfirmedCVD.simple<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVD~crf.ptype+crf.gender+crf.age,family="poisson"))
# 
# modClinEventsConfirmedCVD.univariable<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVD~crf.ptype,family="poisson"))
# 
# modClinEventsConfirmedCVD.univariable.CmzVzv<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVD~crf.ptype+cmvStatus+vzvStatus,family="poisson"))
# 
# modClinEventsConfirmedCVD.simple.CmzVzv<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVD~crf.ptype+crf.gender+crf.age+cmvStatus+vzvStatus,family="poisson"))

modClinEventsConfirmedCVDwithCIMT.simple<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMT~crf.ptype+crf.gender+crf.age,offset=obsTime/(1000*365.25),family="poisson"))

modClinEventsConfirmedCVDwithCIMT.univariable.CmzVzv<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMT~crf.ptype+cmvStatus_2levels+vzvStatus_2levels,offset=obsTime/(1000*365.25),family="poisson"))

modClinEventsConfirmedCVDwithCIMT.simple.CmzVzv<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMT~crf.ptype+crf.gender+crf.age+cmvStatus_2levels+vzvStatus_2levels,offset=obsTime/(1000*365.25),family="poisson"))

modClinEventsConfirmedCVDwithCIMT.full<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMT~crf.ptype+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels,offset=obsTime/(1000*365.25),family="poisson"))

modClinEventsConfirmedCVDwithCIMT.simple<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMT~crf.ptype+crf.gender+crf.age,offset=obsTime/(1000*365.25),family="poisson"))

# modClinEventsConfirmedCVDwithCIMTPWV.simple<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMTPWV~crf.ptype+crf.gender+crf.age,family="poisson"))
# 
# modClinEventsConfirmedCVDwithCIMTPWV.univariable.CmzVzv<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMTPWV~crf.ptype+cmvStatus+vzvStatus,family="poisson"))
# 
# modClinEventsConfirmedCVDwithCIMTPWV.simple.CmzVzv<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantCVDwithCIMTPWV~crf.ptype+crf.gender+crf.age+cmvStatus+vzvStatus,family="poisson"))


modClinEventsSystemic<-with(rhiccaDatWide_impMice.clinEvents,glm(numClinEventsPerParticipantSystemic~crf.ptype+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lum.CD163_enrolment+lum.ICAM1_enrolment+pwv.pwv_enrolment+pwvChange+cimt.cimt_enrolment+cimtChange,family="poisson"))

modClinEventsConfirmedSystemic<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantSystemic~crf.ptype*crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels,family="poisson"))
```
...
rnames<-c(
  ...
  c("Intercept   ","Participant type PLWH   ","Sex male  ","Age (in years)  ","Waist-hip ratio","Diastolic blood pressure (enrolment)","Systolic blood pressure (enrolment)","Glucose (enrolment)","Cholesterol (enrolment)","CMV: persistently high titres  ","VZV: persistently high titres  ")
)
...
# helper functions
funFitAndSum_modClinEventsConfirmedCVD<-function(varString){
  mod<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(as.formula(paste(sep="","numClinEventsPerParticipantCVD~",varString)),offset=obsTime/(1000*365.25),family="poisson"))
  ...
  for(j in 1:nrow(sumObj)){
    sumObj$rrCI[j]<-paste(sep="","(",paste(collapse=",",format(nsmall=3,round(digits=3,c(exp(sumObj$estimateLink[j]-qnorm(0.975)*sumObj$std.error[j]),exp(sumObj$estimateLink[j]+qnorm(0.975)*sumObj$std.error[j]))))),")")
  }
...
funFitAndSum_modClinEventsConfirmedCVDwithCIMT<-function(varString){
  mod<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(as.formula(paste(sep="","numClinEventsPerParticipantCVDwithCIMT~",varString)),offset=obsTime/(1000*365.25),family="poisson"))
...
funFitAndSum_modClinEventsCIMTonly<-function(varString){
  mod<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(as.formula(paste(sep="","numClinEventsPerParticipantCIMTonly~",varString)),offset=obsTime/(1000*365.25),family="poisson"))
...
tab2<-funFitAndSum_modClinEventsConfirmedCVD("crf.ptype+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels")
...
tab2<-funFitAndSum_modClinEventsCIMTonly("crf.ptype+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels")
tab2$term<-as.character(tab2$term)
...
tab2<-funFitAndSum_modClinEventsConfirmedCVDwithCIMT("crf.ptype+crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels")
tab2$term<-as.character(tab2$term)
...
rn<-c("Participant type: PLWH","Sex: male","Age (in years)","Waist-hip ratio","Diastolic blood pressure","Systolic blood pressure","Glucose","Cholesterol","CMV: persistently high titres","VZV: persistently high titres")
...
# helper functions
funFitAndSum_modCimt<-function(varString){
...
funFitAndSum_modClinEventsConfirmedCVD<-function(varString){
  mod<-with(rhiccaDatWide_impMice.clinEventsConfirmed %>% dplyr::filter(crf.ptype=="Patient"),glm(as.formula(paste(sep="","numClinEventsPerParticipantCVD~",varString)),offset=obsTime/(1000*365.25),family="poisson"))
...
funFitAndSum_modClinEventsConfirmedCVDwithCIMT<-function(varString){
  mod<-with(rhiccaDatWide_impMice.clinEventsConfirmed %>% dplyr::filter(crf.ptype=="Patient"),glm(as.formula(paste(sep="","numClinEventsPerParticipantCVDwithCIMT~",varString)),offset=obsTime/(1000*365.25),family="poisson"))
...
funFitAndSum_modClinEventsCIMTonly<-function(varString){
  mod<-with(rhiccaDatWide_impMice.clinEventsConfirmed %>%  dplyr::filter(crf.ptype=="Patient"),glm(as.formula(paste(sep="","numClinEventsPerParticipantCIMTonly~",varString)),offset=obsTime/(1000*365.25),family="poisson"))
...
tab1<-rbind(tab1.baseline,
            ...
            tab1.waisthip %>% dplyr::filter(term!="cimt.cimt_enrolment"),
...
tab2<-funFitAndSum_modClinEventsConfirmedCVD("crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months")
tab2$term<-as.character(tab2$term)

tab2.nocd4<-funFitAndSum_modClinEventsConfirmedCVD("crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels")
...
tab2<-funFitAndSum_modClinEventsCIMTonly("crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months")
tab2$term<-as.character(tab2$term)

tab2.nocd4<-funFitAndSum_modClinEventsCIMTonly("crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels")
tab2.nocd4$term<-as.character(tab2.nocd4$term)
...
tab2<-funFitAndSum_modClinEventsConfirmedCVDwithCIMT("crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels+lims.CD4_Absolute_enrolment+lims.CD4_Absolute_changeFromBaselineAt24Months")
tab2$term<-as.character(tab2$term)

tab2.nocd4<-funFitAndSum_modClinEventsConfirmedCVDwithCIMT("crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels")
tab2.nocd4$term<-as.character(tab2.nocd4$term)
...
rn<-c("Sex: male","Age (in years)","Waist-hip ratio","Diastolic blood pressure","Systolic blood pressure","Glucose","Cholesterol","CMV: persistently high titres","VZV: persistently high titres","CD4 (baseline)","CD4 (change from baseline at 24 months)")

tabAll %>%
  dplyr::select(!allRows) %>%
  kableRn(digits=3,escape=F,row_names=rn,col.names=c(rep(c("RR","95% CI","p"),2),rep(c("RR","95% CI","p"),4)),caption="Model summaries for models for PLWH only 1) fitted to clinical CVD events only, 2) fitted to sub-clinical CIMT events only and 3) fitted to clinical CVD events and sub-clinical CIMT events (both unadjusted, single variable models and multiple regression models). The event count models are Poisson models and hence report rate ratios. Models fitted to multiply-imputed data from PLWH only.") %>%
  #pack_rows(index = c("Unadjusted models, all data" = 3, "Multiple regression model, all data" = 16, "Multiple regression model, PLWH only" = 18)) %>%
  add_header_above(header=c(" "=1,"Unadjusted models" = 3, "Multiple regression model" = 3,"Unadjusted models" = 3, "Multiple regression model" = 3,"Unadjusted models" = 3, "Multiple regression model" = 3)) %>%
  add_header_above(header=c(" "=1,"Clinical CVD events" = 6, "Sub-clinical CIMT events" = 6,"Clinical CVD and sub-clinical CIMT events" = 6)) %>%
  kable_styling(full_width=F)
...
tabAll.PLWH.cd4<-tabAll[,c(1,14:19)]
...
tabAll$clineventwithcimt_pvalue.unadj<-as.numeric(gsub(pattern="</span>",replacement="",gsub(pattern="<span.*>0",replace="0",tabAll$clineventwithcimt_pvalue.unadj)))
tabAll$clineventwithcimt_pvalue.adj<-as.numeric(gsub(pattern="</span>",replacement="",gsub(pattern="<span.*>0",replace="0",tabAll$clineventwithcimt_pvalue.adj)))
write.csv(tabAll,row.names=F,file=paste(sep="_",outPrefix,"CIMT_CVDevents_PLWH_Table3.csv"))
```

### Without CD4

```{r}
rn.nocd4<-c("Sex: male","Age (in years)","Waist-hip ratio","Diastolic blood pressure","Systolic blood pressure","Glucose","Cholesterol","CMV: persistently high titres","VZV: persistently high titres")
...
write.csv(tabAll.nocd4,row.names=F,file=paste(sep="_",outPrefix,"CIMT_CVDevents_PLWH_Table3_nocd4.csv"))
```
...
For the sensitivity analysis we will manually impute all missing VZV statuses to "not persistently high VZV titres" (scenario 1 - *best case*) and to "persistently high VZV titres" (scenario 2 - *worst case*) respectively, and similarly for the sensitivity analysis for CMV status.

```{r sensitivity}
funFitAndSum_modClinEventsConfirmedCVDwithCIMT_SA<-function(varString,exp,scen){
  tt<-complete(rhiccaDatWide_impMice.clinEventsConfirmed,include=TRUE,action="long")
  idxMVvzv<-which(is.na(tt$vzvStatus_2levels[tt$.imp==0]))
  idxMVcmv<-which(is.na(tt$cmvStatus_2levels[tt$.imp==0]))
  for(m in 1:max(tt$.imp)){
    if(scen=="worst"){
      if(exp=="vzv"){tt$vzvStatus_2levels[tt$.imp==m][idxMVvzv]<-"persistently high VZV titres"}
      if(exp=="cmv"){tt$cmvStatus_2levels[tt$.imp==m][idxMVcmv]<-"persistently high CMV titres"}
    }
    if(scen=="best"){
      if(exp=="vzv"){tt$vzvStatus_2levels[tt$.imp==m][idxMVvzv]<-"not persistently high VZV titres"}
      if(exp=="cmv"){tt$cmvStatus_2levels[tt$.imp==m][idxMVcmv]<-"not persistently high CMV titres"}
    }
  }
  tt<-as.mids(tt)
  
  mod<-with(tt %>% dplyr::filter(crf.ptype=="Patient"),glm(as.formula(paste(sep="","numClinEventsPerParticipantCVDwithCIMT~",varString)),offset=obsTime/(1000*365.25),family="poisson"))
...
tab2.nocd4.sa.worst.vzv<-funFitAndSum_modClinEventsConfirmedCVDwithCIMT_SA("crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels",scen="worst",exp="vzv")
tab2.nocd4.sa.worst.vzv$term<-as.character(tab2.nocd4.sa.worst.vzv$term)

tab2.nocd4.sa.best.vzv<-funFitAndSum_modClinEventsConfirmedCVDwithCIMT_SA("crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels",scen="best",exp="vzv")
tab2.nocd4.sa.best.vzv$term<-as.character(tab2.nocd4.sa.best.vzv$term)

tab2.nocd4.sa.worst.cmv<-funFitAndSum_modClinEventsConfirmedCVDwithCIMT_SA("crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels",scen="worst",exp="cmv")
tab2.nocd4.sa.worst.cmv$term<-as.character(tab2.nocd4.sa.worst.cmv$term)

tab2.nocd4.sa.best.cmv<-funFitAndSum_modClinEventsConfirmedCVDwithCIMT_SA("crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+cmvStatus_2levels+vzvStatus_2levels",scen="best",exp="cmv")
...
tab2.nocd4.sa.best.vzv %>%
  kableRn(digits=3,escape=F,caption="BEST case scenario: every missing VZV status = not persistently high VZV. The association with VZV remains statistically significant - in fact becomes slightly stronger.") %>%
  kable_styling() 
```
...
tab2.nocd4.sa.best.cmv %>%
  kableRn(digits=3,escape=F,caption="BEST case scenario: every missing CMV status = not persistently high CMV. The association with CMV remains insignificant (and not that the association with VZV remains largely unchanged).") %>%
  kable_styling()
```

### Table 3 for manuscript

```{r combineTablesforTable3OfManuscript}
rnames<-c("crf.ptypePatient",tabAll.PLWH.nocd4[,1],"lims.CD4_Absolute_enrolment","lims.CD4_Absolute_changeFromBaselineAt24Months")
rn<-c("Participant type: PLWH","Sex: male","Age (years)","Waist-hip ratio","Diastolic blood pressure","Systolic blood pressure","Glucose (baseline)","Cholesterol (baseline)","CMV: persistently high titres","VZV: persistently high titres","CD4 (baseline)","CD4 (change from baseline at 24 months)")
...
grid.arrange(nrow=1,gCIMTAdj.notitle+ggtitle("A. Pre-clinical CVD (CIMT)"),gPWVAdj.notitle+ggtitle("B. Pre-clinical CVD (PWV)"))
...
modClinEventsConfirmedSystemic<-with(rhiccaDatWide_impMice.clinEventsConfirmed,glm(numClinEventsPerParticipantSystemic~crf.ptype+crf.gender+crf.age+physio.waisthip_enrolment+cmvStatus+vzvStatus,offset=obsTime/(1000*365.25),family="poisson"))
...
This is confirmed by computing and bootstrapping the ACME: the 95% confidence interval includes 0, showing no evidence to support a mediation effect.
...
modOuts %>%
  ...
  kableRn(row_names=NULL,row.names=F,escape=F,caption=paste(sep="","Mediation analysis for the effect of participant type through change-from-baseline ICAM-1 on change-from-baseline CIMT. The 95% confidence interval for ACME is given by: (",paste(collapse=", ",format(nsmall=3,round(digits=3,acmeCI))),").")) %>%
  kable_styling(full_width = FALSE) %>%
...
for(b in 1:B){
  ...
  bDirTot<-modYX$estimate[modYX$term=="vzvStatuspersistently high VZV titres"]
  bDirMed<-modYMX$estimate[modYMX$term=="vzvStatuspersistently high VZV titres"]
  acme[b]<-bDirTot-bDirMed
}
...
modOuts %>%
  ...
  dplyr::select(coefficient,estimate,std.error,statistic,df,p.value) %>%
  mutate(p.value=ifelse(coefficient!="Intercept" & p.value<0.05,cell_spec(format(nsmall=4,round(digits=4,p.value)),bold=T),cell_spec(format(nsmall=4,round(digits=4,p.value)),bold=F))) %>%
  kableRn(row_names=NULL,row.names=F,escape=F,caption=paste(sep="","Mediation analysis for the effect of VZV status through baseline inflammatory markers on change-from-baseline CIMT (PLWH only). The 95% confidence interval for ACME is given by: (",paste(collapse=", ",format(nsmall=3,round(digits=3,acmeCI))),").")) %>%
  kable_styling(full_width = FALSE) %>%
...
for(b in 1:B){
  ...
  modYX<-summary(pool(with(datBoot2,glm(numClinEventsPerParticipantCVDwithCIMT~crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+vzvStatus,offset=obsTime/(1000*365.25),family="poisson"))))
  modM1X<-summary(pool(with(datBoot,glm(log(lum.ICAM1_enrolment)~vzvStatus))))
  modM2X<-summary(pool(with(datBoot,glm(log(lum.CD163_enrolment)~vzvStatus))))
  modM3X<-summary(pool(with(datBoot,glm(log(lum.IL6_enrolment)~vzvStatus))))
  modYMX<-summary(pool(with(datBoot2,glm(numClinEventsPerParticipantCVDwithCIMT~crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+vzvStatus+log(lum.ICAM1_enrolment)+log(lum.CD163_enrolment)+log(lum.IL6_enrolment),offset=obsTime/(1000*365.25),family="poisson"))))
...
modYX<-summary(pool(with(rhiccaDatWide_impMice.clinEventsConfirmed %>% dplyr::filter(crf.ptype=="Patient"),glm(numClinEventsPerParticipantCVDwithCIMT~crf.gender+crf.age+physio.waisthip_enrolment+physio.bp_dia_enrolment+physio.bp_sys_enrolment+lims.Glucose_enrolment+lims.Cholesterol_enrolment+vzvStatus,offset=obsTime/(1000*365.25),family="poisson"))))
modM1X<-summary(pool(with(rhiccaDatWide_impMice.hiv,glm(log(lum.ICAM1_enrolment)~vzvStatus))))
modM2X<-summary(pool(with(rhiccaDatWide_impMice.hiv,glm(log(lum.CD163_enrolment)~vzvStatus))))
modM3X<-summary(pool(with(rhiccaDatWide_impMice.hiv,glm(log(lum.IL6_enrolment)~vzvStatus))))
...
modOuts %>%
  mutate(
    coefficient=c(c("Intercept","Sex male","Age","Waist-hip ratio","Diastolic blood pressure","Systolic blood pressure","Glucose (baseline)","Cholesterol (baseline)","VZV: progression to latency","VZV: reactivation","VZV: persistently high titres"),rep(c("Intercept","VZV: progression to latency","VZV: reactivation","VZV: persistently high titres"),3),c("Intercept","Sex male","Age","Waist-hip ratio","Diastolic blood pressure","Systolic blood pressure","Glucose (baseline)","Cholesterol (baseline)","VZV: progression to latency","VZV: reactivation","VZV: persistently high titres","log ICAM-1 (baseline)","log CD163 (baseline)","log IL-6 (baseline)"))
  ) %>%
  dplyr::select(coefficient,estimate,std.error,statistic,df,p.value) %>%
  mutate(p.value=ifelse(coefficient!="Intercept" & p.value<0.05,cell_spec(format(nsmall=4,round(digits=4,p.value)),bold=T),cell_spec(format(nsmall=4,round(digits=4,p.value)),bold=F))) %>%
  kableRn(row_names=NULL,row.names=F,escape=F,caption=paste(sep="","Mediation analysis for the effect of VZV status through baseline inflammatory markers on clinical and sub-clinical (CIMT) CVD events. The 95% confidence interval for ACME is given by: (",paste(collapse=", ",format(nsmall=3,round(digits=3,acmeCI))),").")) %>%
  kable_styling(full_width = FALSE) %>%
